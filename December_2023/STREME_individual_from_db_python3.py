    ...
    with open(filename, 'r') as infile:
        while True:
            ...
            if line.startswith('MOTIF'):
                ...
                with open(outfilename_prefix + '_streme.txt', 'w') as outfile:
            ...
            if line.startswith('letter-probability matrix:'):# or ('log-odds matrix'):
                with open(outfilename_prefix + '_streme.txt', 'a') as outfile:
                    outfile.write(line)
                    while True:
                        next_line = infile.readline()
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt in ('-i', '--input'):
            ...
            print('python ~/STREME_individual_from_db.py -i combined_motif_db.txt')
            sys.exit()
    if name:
        matrix(name)
    else:
