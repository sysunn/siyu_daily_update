...
python MEME_individual_from_db_python3.py ../GATA3_without_motif123_top10000_output_1/meme.txt
...
```{r, engine='bash', eval=F, echo=TRUE}
mast -hit_list -best individual_meme/GATAmotif4_meme.txt without_motifs_123.fasta > mast_GATA3_PSWM_in_peaks_round4.txt
...
#wget https://raw.githubusercontent.com/sysunn/siyu_daily_update/main/December_2023/MEME_individual_from_db_python3.py
python MEME_individual_from_db_python3.py ../GATA3_without_motif1234_top10000_output/meme.txt
...
```{r, engine='bash', eval=F, echo=TRUE}
#wget https://raw.githubusercontent.com/sysunn/siyu_daily_update/main/December_2023/MEME_individual_from_db_python3.py
python MEME_individual_from_db_python3.py ../GATA3_without_motif12345_top10000_output/meme.txt
...
```{r, engine='bash', eval=F, echo=TRUE}
#wget https://raw.githubusercontent.com/sysunn/siyu_daily_update/main/December_2023/MEME_individual_from_db_python3.py
python MEME_individual_from_db_python3.py GATA3_without_123456_streme_output/streme.txt
cp 4-AGATAAM_meme.txt individual_meme/AGATAAM_streme.txt
```
...
```{r, engine='bash', eval=F, echo=TRUE}
#wget https://raw.githubusercontent.com/sysunn/siyu_daily_update/main/December_2023/MEME_individual_from_db_python3.py
python MEME_individual_from_db_python3.py GATA3_without_123456_7_streme_output/streme.txt
cp 6-TGATAA_meme.txt individual_meme/TGATAA_streme.txt
```
...
```
Replace the central 81 bp sequences to 50 Ns. \
...
#SBATCH -o streme8N.sh_%j.out
#SBATCH -e streme8N.sh_%j.err
...
# MEME-classic
meme -p 192 -oc GATA3_without_motifs_123456_78_161bpwindow_81NNN_meme_output -nmotifs 5 -objfun classic -csites 20000 -searchsize 0 -minw 4 -maxw 20 -revcomp -dna -bfile hg38_bkgrnd.txt -maxsize 100000000 without_motifs_123456_78_161bpwindow_81NNN.fasta 
```

## MEME-10,000 peaks

In case the above MEME takes a very long time to run, I am also subsetting top 10,000 peaks to perform MEME. \
```{r, engine='bash', eval=F, echo=TRUE}
# select top 10000
...
```{r, engine='bash', eval=TRUE, echo=TRUE}
wc -l Exhaustive_MEME_MAST_round1to8/without_motifs_123456_78_161bpwindow_top10000.bed
head -3 Exhaustive_MEME_MAST_round1to8/without_motifs_123456_78_161bpwindow_top10000.bed
wc -l Exhaustive_MEME_MAST_round1to8/without_motifs_123456_78_161bpwindow_top10000.fasta
head -6 Exhaustive_MEME_MAST_round1to8/without_motifs_123456_78_161bpwindow_top10000.fasta
```
Replace the central 81 bp sequences to 50 Ns. \
```{r, engine='bash', eval=F, echo=TRUE}
#161window flank with central 81bp substituted with 50 Ns. 
awk '!/^>/ { mid = int(length($0) / 2); $0 = substr($0, 1, mid - 41) "NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN" substr($0, mid + 41); } 1' without_motifs_123456_78_161bpwindow_top10000.fasta > without_motifs_123456_78_161bpwindow_81NNN_top10000.fasta
```

```{r, engine='bash', eval=TRUE, echo=TRUE}
head -8 Exhaustive_MEME_MAST_round1to8/without_motifs_123456_78_161bpwindow_top10000.fasta
head -8 Exhaustive_MEME_MAST_round1to8/without_motifs_123456_78_161bpwindow_81NNN_top10000.fasta
```
```{r, engine='bash', eval=F, echo=TRUE}
#!/bin/bash
...
To save time and run job in parallel, I will divide this file into two files each with 5k sequences. \

```{r, engine='bash', eval=F, echo=TRUE}
head -10000 without_motifs_123456_78_161bpwindow_81NNN_top10000.fasta > without_motifs_123456_78_161bpwindow_81NNN_top10000_1.fasta
tail -10000 without_motifs_123456_78_161bpwindow_81NNN_top10000.fasta > without_motifs_123456_78_161bpwindow_81NNN_top10000_2.fasta
```

```{r, engine='bash', eval=F, echo=TRUE}
#!/bin/bash
#SBATCH --job-name=MEME8N_1.1.sh    
#SBATCH -N 6
#SBATCH -n 6
#SBATCH -c 16
#SBATCH -p general
...
#SBATCH --mail-type=ALL
#SBATCH --mail-user=ssun@uchc.edu
#SBATCH -o MEME8N_1.sh_%j.out
#SBATCH -e MEME8N_1.sh_%j.err
...
# MEME-classic
meme -p 96 -oc GATA3_without_motifs_123456_78_161bpwindow_81NNN_top10000_meme_output_1 -nmotifs 5 -objfun classic -csites 20000 -searchsize 0 -minw 4 -maxw 20 -revcomp -dna -bfile hg38_bkgrnd.txt -maxsize 100000000 without_motifs_123456_78_161bpwindow_81NNN_top10000_1.fasta
```   

```{r, engine='bash', eval=F, echo=TRUE}
#!/bin/bash
#SBATCH --job-name=MEME8N_1.2.sh    
#SBATCH -N 8
#SBATCH -n 8
#SBATCH -c 20
#SBATCH -p general
...
#SBATCH --mail-type=ALL
#SBATCH --mail-user=ssun@uchc.edu
#SBATCH -o MEME8N_1.2.sh_%j.out
#SBATCH -e MEME8N_1.2.sh_%j.err
...
# MEME-classic
meme -p 160 -oc GATA3_without_motifs_123456_78_161bpwindow_81NNN_top10000_meme_output_2 -nmotifs 5 -objfun classic -csites 20000 -searchsize 0 -minw 4 -maxw 20 -revcomp -dna -bfile hg38_bkgrnd.txt -maxsize 100000000 without_motifs_123456_78_161bpwindow_81NNN_top10000_2.fasta
```

# Next step

Figure out how to deal with the previous 8 PSWMs and the expanded regions. \
First concatenate previous 8 PSWMs into a single file (from code). \
Then run the concatenated 8 PSWMs with `MAST` on the expanded region. \
Coherence check: For the expanded region--161bp (without swapping central to N), the central (~100bp) region should have no hits for the 8 previous PSWMs. \
...
#rm *meme.txt #remove the non-used individual motifs
```
...
python MEME_concatenate_motifs_to_db_python3.py individual_meme/ cat_8_motifs.txt
```

```{r, engine='bash', eval=TRUE, echo=TRUE}
head -20 Exhaustive_MEME_MAST_round1to8/cat_8_motifs.txt
```
