...
for (chip.peak in Sys.glob(file.path("./closest.2nd.distance.quantile*.bed"))) {
    ...
    all.distance = cbind(read.table(chip.peak,header=F, comment.char='')[,c(1:3, 9, 11)], quantile.name)
    df.chip = rbind(df.chip, all.distance)
}
str(df.chip)
unique(df.chip$quantile.name)
...
densityplot( ~ abs(dis),
            ...
            data = df.all,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
            col=c("grey60", colorRampPalette(c("pink","red"))(4), "#ce228e"),
            aspect = 1,
            xlim=c(0,50),
            from=0,
            to=50,
            ...
            lty = c(1),
            lwd=2,
            par.settings = list(superpose.line = list(col=c("grey60", colorRampPalette(c("pink","red"))(4), "#ce228e"), lwd=3), strip.background=list(col="grey85")),
         panel = function(...) {
             panel.abline(v= 9, lty =2)
             panel.abline(v= 2, lty =2)
             panel.densityplot(...)
...
In the above plot, it is important to set `from` and `to` arguments inside `density()`. Because by default, density will extend the range so that the density curve approaches 0 at the extreme. \
...
CTCF peak sets (remove motifs 123456789) \
