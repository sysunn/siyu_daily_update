...
`-A` option completely remove a feature from `-a file` if it has even 1bp of overlap with a feature in `-b file`. \
...
bedtools subtract -a hg38.3.3.3.30_plus_minus_GAT.sorted.bed -b closest_1st_GAT_to_GATA_uniq_peak.sorted.bed -A -s > hg38.3.3.3.30_plus_minus_GAT_without_1st_closest_GAT.bed
...
```{r engine='R',class.source = "revision", eval=F, echo=TRUE}
dir=/labs/Guertin/siyu/Sathyan_GATA3_ChIP_pool1_pool2/overrep_3mer/hg38_full_kmer3_rs30/seqdump/
sort -k1,1 -k2,2n ${dir}hg38.3.3.3plus.36_GAT.bed > hg38.3.3.3plus.36_GAT.sorted.bed #32147038
sort -k1,1 -k2,2n ${dir}hg38.3.3.3minus.14_GAT.bed  > hg38.3.3.3minus.14_GAT.sorted.bed #32096009



awk '$6 == "+" {print > "closest_1st_plus_GAT_to_GATA_peak.sorted.bed"}' closest_1st_GAT_to_GATA_peak.sorted.bed  #3873
awk '$6 == "-" {print > "closest_1st_minus_GAT_to_GATA_peak.sorted.bed"}' closest_1st_GAT_to_GATA_peak.sorted.bed  #3923

bedtools subtract -a hg38.3.3.3plus.36_GAT.sorted.bed -b closest_1st_plus_GAT_to_GATA_peak.sorted.bed -A -s > hg38.3.3.3.30_plus_GAT_without_1st_closest_GAT.bed #32143168
bedtools subtract -a hg38.3.3.3minus.14_GAT.sorted.bed -b closest_1st_minus_GAT_to_GATA_peak.sorted.bed -A -s > hg38.3.3.3.30_minus_GAT_without_1st_closest_GAT.bed #32092088

```
minus has 2 not subtract, plus has 3 \

```{r engine='R',class.source = "revision", eval=F, echo=TRUE}
bedtools subtract -a hg38.3.3.3plus.36_GAT.sorted.bed -b hg38.3.3.3.30_plus_GAT_without_1st_closest_GAT.bed -s > test_plus.bed #3870
bedtools subtract -a closest_1st_plus_GAT_to_GATA_peak.sorted.bed -b test_plus.bed -s > test_diff_plus.bed

sort closest_1st_plus_GAT_to_GATA_peak.sorted.bed | uniq -d
chr12	52949087	52949090	36	36	+	GAT
chr3	146161582	146161585	36	36	+	GAT
chr8	101493974	101493977	36	36	+	GAT 
```
due to duplicated regions 
-->
...
```{r engine='R', class.source = "revision", eval=TRUE, echo=TRUE}
library(bigWig)
chip.peak=read.table("closest_1st_GAT_to_GATA_uniq_peak.sorted.bed", header=FALSE)
nrow(chip.peak)
head(chip.peak)
...
```{r engine='R',class.source = "revision", eval=F, echo=TRUE}
closest.2nd.distance=bedTools.closest(bed1 = chip.peak.summit[,1:3], bed2 = all.GAT.without.1st.closest.file, opt.string = '-d -t first')
...
nrow(closest.2nd.distance)
nrow(closest.2nd.distance[closest.2nd.distance$V9=="+",])
nrow(closest.2nd.distance[closest.2nd.distance$V9=="-",])
```
```{r engine='R',class.source = "revision", eval=TRUE, echo=TRUE}
df.GATA.2nd.distance = cbind(closest.2nd.distance[,c(1:3, 11)], "GATA3_peak")
colnames(df.GATA.2nd.distance) = c(colnames(df.GATA.2nd.distance)[1:3], "dis", "status")
head(df.GATA.2nd.distance)
```

```{r engine='R',class.source = "revision", eval=TRUE, echo=TRUE}
library(lattice)
densityplot(~ abs(df.GATA.2nd.distance$dis), 
            xlim=c(0,50),
            main = "Probability Density Function Plot", 
            xlab = "distance from 2nd GAT to 1st GAT")
```

The Probability Density Function Plot shows that most 2nd closest GAT are found at ~10bp to the 1st closest GAT. This suggest that in GATA ChIP peaks, 3mer GAT-GAT are often found to have a fixed spacings between them. \


#### unionHDS set (without the 9 GATA3-like motifs)
