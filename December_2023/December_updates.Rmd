...
**Notice that if `closestBed` found overlap features, ethan the distance will be reported as 0; if no overlaps are found, `closestBed` looks for the feature in B that is closest (that is, least genomic distance to the start or end of A) to A.** \
...
Since `closestBed` add 1 in its calculation of distance, we need to know that if we want to know the actual fixed spacings between two 3mer, we use the distance -1. \
