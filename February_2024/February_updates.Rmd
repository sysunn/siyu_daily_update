...
for (i in 1:length(prioritized_triplets)) {
  for (j in 1:length(prioritized_triplets)) {
    ...
    sixmer_DNAString <- DNAString(sixmer)
    
    # Find reverse complement of the 6mer
    reverse_complement_sixmer <- reverseComplement(sixmer_DNAString)
  
    # Check if the reverse complement exists in the 6mer itself
   if (!as.character(reverse_complement_sixmer)  %in% non_redundant_6mers) {
...
There are 105 combinations of 3mer pairs, these pairs are unique when all of them are considered to be on the same strand. \
...
for (motif in my_motifs) {
  ...
  for (i in 1:nrow(df)){
    ...
    print(pattern1)
    closest_plus_3mer_to_GATA3_peak_summits=fiveprime.bed(read.table(paste0(dir1, "closest.1st.plus.",pattern1, ".to.GATA3.with.", motif, ".bed"), header=FALSE), upstreamWindow = win[win$pattern==pattern1, "plus_upstream"] , downstreamWindow = win[win$pattern==pattern1, "plus_downstream"])
...
for (i in 1:nrow(df)){
    ...
    # anchor position: closest +/- pattern1 relative to G/C
    print(pattern1)
    closest_plus_3mer_to_DHS=fiveprime.bed(read.table(paste0(dir1, "closest.1st.plus.", pattern1, ".to.indep.DHS.control.consensus.bed"), header=FALSE), upstreamWindow = win[win$pattern==pattern1, "plus_upstream"] , downstreamWindow = win[win$pattern==pattern1, "plus_downstream"])
...
#### plot the enrichement frequencies with xy plots
Make minimal plots to communicate results. \

For each peak set, (5 positive GATA3 peak sets and one DHS independent regions), there are 105 unique 3mer pairs with different relative distances. \
...
RSAT:
Confirm how the software counts the observed spaced 3mer. Will they count twice if seeing 2 spaced 3mer at different locus on one sequence?
