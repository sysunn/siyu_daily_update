...
I will utilize `MAST` against the 161bp window peaks for all 8 motifs (the concatenated database file) to identify motif coordinates corresponding to each motif. Subsequently, I'll employ `intersectBed` to isolate the peak sets containing the best motif within the 161bp window. The peak sets characterized by the presence of the specific motif structures that we are interested in will serve as positive controls. \
...
We can determine the fraction of peaks with closest GAT: \

```{r engine='R', eval=TRUE, echo=TRUE}
frac1=sum(df1[df1$status=="pos.control.motif.all",]$dis <= 16) / length(df1[df1$status=="pos.control.motif.all",]$dis)
frac2=sum(df.neg.consensus[df.neg.consensus$status=="full.DHS.control",]$dis <= 16) / length(df.neg.consensus[df.neg.consensus$status=="full.DHS.control",]$dis)
frac3=sum(df.neg.consensus[df.neg.consensus$status=="indep.DHS.control",]$dis <= 16) / length(df.neg.consensus[df.neg.consensus$status=="indep.DHS.control",]$dis)
frac1-frac2
frac1-frac3
```
...
```{r engine='R',  eval=TRUE, echo=TRUE}
ecdf(df1[df1$status=="pos.control.motif.all",]$dis)(16)
```
...
for (folder_name in folder_names) {
  ...
  print(densityplot( ~ abs(dis),
            ...
            par.settings = list(superpose.line = list(col=c("red", "blue4","blue"), lwd=3), strip.background=list(col="grey85")),
            panel = function(...) {
             panel.abline(v= 0, lty =2, col="red")
             panel.abline(v= 16, lty =2,col="red")
             panel.densityplot(...)
  ...
  print(ecdfplot(~log(abs(dis), base = 10), groups = status, data = df.plot,
        ...
        #xlim = c(0, 50000),
         #scales=list(relation="free",alternating=c(1,1,1,1)),
         main=paste0("3mer: ", folder_name),
         ylab = 'Cumulative Distribution Function',
         xlab = expression('log'[10]~ '3mer Distance from peak summit'),
                                        #index.cond = list(c(2,1)),
         between=list(y=1.0),
         type = 'a',
         xlim = c(0,2.5),
         ...
         par.settings = list(superpose.line = list(col=c("red", "blue4","blue"), lwd=3), strip.background=list(col="grey85")),
         panel = function(...) {
             panel.abline(v= 1.204, lty =2, col="red") #log10(16)=1.204
             panel.ecdfplot(...)
...
To quantitatively compare the differences of 3mer enrichment near summit regions within range (0,16)(bp) between GATA3 peaks and DHS regions, we can calculate the differences of AUC within the range we selected. \
```{r engine='R',  eval=F, echo=TRUE}
#module load R/4.1.2
#R
directory_path <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer"
...
```{r engine='R', eval=TRUE, echo=TRUE}
AUC_df <- read.table("AUC_values.txt", header = FALSE, sep = "\t")
colnames(AUC_df)=c("triplet", "prox_enrich_16bp_pos", "prox_enrich_16bp_neg_full", "prox_enrich_16bp_neg_indep", "prox_enrich_16bp_ave_difference")
# Sort the data frame based on the fifth column in descending order
sorted_AUC_df <- AUC_df[order(-AUC_df$prox_enrich_16bp_ave_difference), ]
sorted_AUC_df
```


calculate the fraction of peaks has closer 3mer by CDF. \
```{r engine='R',  eval=F, echo=TRUE}
#module load R/4.1.2
#R
directory_path <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer"
output_file <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer/CDF_fractions.txt"
output_connection <- file(output_file, "w") # Open the output file for writing
folder_names <- apply(expand.grid(c("A", "T", "C", "G"), c("A", "T", "C", "G"), c("A", "T", "C", "G")), 1, paste, collapse = "")
#folder_names <- list.dirs(directory_path, full.names = FALSE) # Get a list of folder names in the directory
...
for (folder_name in folder_names) {
  ...
  #get the fraction of regions with closer 3mer for each status
  
  pos.peak=ecdf(df.plot[df.plot$status=="pos",]$dis)(16)
  neg.full.peak=ecdf(df.plot[df.plot$status=="neg.full",]$dis)(16)
  neg.indep.peak=ecdf(df.plot[df.plot$status=="neg.indep",]$dis)(16)
  
  #get the average difference between fraction of pos.peak vs. neg.peak
  fraction_difference <- ((pos.peak - neg.full.peak)+(pos.peak - neg.indep.peak))/2
  
  # Print the auc_difference value as needed
  cat(sprintf("%s\t%s\t%s\t%s\t%s\n", folder_name, pos.peak, neg.full.peak, neg.indep.peak, fraction_difference), file = output_connection)
...
Rank the fraction of peaks with closer 3mer from largest difference to smallest difference. \

```{r engine='R',  eval=TRUE, echo=TRUE}
CDF_df <- read.table("CDF_fractions.txt", header = FALSE, sep = "\t")
colnames(CDF_df)=c("triplet", "prox_enrich_16bp_pos", "prox_enrich_16bp_neg_full", "prox_enrich_16bp_neg_indep", "prox_enrich_16bp_ave_difference")
# Sort the data frame based on the fifth column in descending order
sorted_CDF_df <- CDF_df[order(-CDF_df$prox_enrich_16bp_ave_difference), ]
sorted_CDF_df
```

**scatter plot** \

The intersection point of CDF is essentially same with the AUC of PDF at distance(x)==16bp. \
The scatter plot of the y intersection of CDF at x==16bp and the AUC of PDF at x==16bp is on the diagonal. 
```{r engine='R',  eval=TRUE, echo=TRUE}
library(lattice)
xyplot(AUC_df$prox_enrich_16bp_ave_difference ~CDF_df$prox_enrich_16bp_ave_difference, 
     asp=1,
     xlim=c(-0.5,0.5),
     ylim=c(-0.5,0.5))
```

**determine categories for 3mer enrichment in proximity to GATA3 peaks** \

We've compiled a list of 3-mers ranked by their proximity to GATA3 peaks. Our next step involves clustering these 3-mers based on their enrichment levels. To gain insights, we'll use visual plots depicting the distribution of 3-mer enrichment. \
Since in the scatter plot above, it is very similar for using AUC from PDF plots or y intersection from CDF plots at the defined distance. From now on, I will only use the y values from CDF plots at defined distance to determine "proximal enrichment". \
...
Barchart of GATA3 peak\
```{r engine='R', eval=TRUE, echo=TRUE}
CDF_df_rank_pos=CDF_df[order(-CDF_df$prox_enrich_16bp_ave_difference), ][,c(1,5)]
# Reorder levels of the factor variable based on fraction
CDF_df_rank_pos$triplet<- factor(CDF_df_rank_pos$triplet, 
                                 levels = CDF_df_rank_pos$triplet[order(CDF_df_rank_pos$prox_enrich_16bp_ave_difference, decreasing = TRUE)])
head(CDF_df_rank_pos)
...
barchart(prox_enrich_16bp_ave_difference ~ triplet,
          #groups=triplet,
          horizontal = FALSE,
          data=CDF_df_rank_pos,
          #type="count",
          #layout = c(1, length(unique(CDF_df_rank_pos$triplet))),
          #ylim=c(-0,0.5),
          col=ifelse(CDF_df_rank_pos$prox_enrich_16bp_ave_difference >= 0, "skyblue", "grey"),
          origin = 0,
          scales = list(x = list(cex = 0.5, rot = 45)),
          xlab="3mer",
          ylab="fraction of proximal emrichment",
          main="3mer Proximal Emrichment to GATA3 peaks at 16bp",
          cex.axis=0.5,
          ...
          panel=function(...) {
           panel.barchart(...)
           panel.abline(h=0.20, lty = "dotted", col = "red", identifier = "abline")
           panel.abline(h=0, lty = "dotted", col = "red", identifier = "abline")
       }
          )
```

I want to put cut-offs at fration==0.20, and fraction==0; The 3mers that has a proximal enrichment larger than 0.15 will be defined as "high enrichment", between 0 to 0.15 is "low enrichment", fraction below 0 is "no enrichment". \


**test robustness of our categorization** \
The fraction we plot in the above plots are using distance at 16bp to define 3mer enrichment "near" GATA3 peaks. We will now plotting using distance at 20bp, 30bp, and 50bp to see if we can get robust results for clustering. \
...
#R
directory_path <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer"
output_file <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer/CDF_fractions_20bp.txt"
output_connection <- file(output_file, "w") # Open the output file for writing
folder_names <- apply(expand.grid(c("A", "T", "C", "G"), c("A", "T", "C", "G"), c("A", "T", "C", "G")), 1, paste, collapse = "")
#folder_names <- list.dirs(directory_path, full.names = FALSE) # Get a list of folder names in the directory
...
for (folder_name in folder_names) {
  ...
  pos.peak=ecdf(df.plot[df.plot$status=="pos",]$dis)(20)
  neg.full.peak=ecdf(df.plot[df.plot$status=="neg.full",]$dis)(20)
  neg.indep.peak=ecdf(df.plot[df.plot$status=="neg.indep",]$dis)(20)
  
  #get the average difference between fraction of pos.peak vs. neg.peak
  fraction_difference <- ((pos.peak - neg.full.peak)+(pos.peak - neg.indep.peak))/2
  
  # Print the CDF_fraction_difference value as needed
  cat(sprintf("%s\t%s\t%s\t%s\t%s\n", folder_name, pos.peak, neg.full.peak, neg.indep.peak, fraction_difference), file = output_connection)
...
#R
directory_path <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer"
output_file <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer/CDF_fractions_30bp.txt"
output_connection <- file(output_file, "w") # Open the output file for writing
folder_names <- apply(expand.grid(c("A", "T", "C", "G"), c("A", "T", "C", "G"), c("A", "T", "C", "G")), 1, paste, collapse = "")
#folder_names <- list.dirs(directory_path, full.names = FALSE) # Get a list of folder names in the directory
...
for (folder_name in folder_names) {
  ...
  pos.peak=ecdf(df.plot[df.plot$status=="pos",]$dis)(30)
  neg.full.peak=ecdf(df.plot[df.plot$status=="neg.full",]$dis)(30)
  neg.indep.peak=ecdf(df.plot[df.plot$status=="neg.indep",]$dis)(30)
...
#R
directory_path <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer"
output_file <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer/CDF_fractions_40bp.txt"
output_connection <- file(output_file, "w") # Open the output file for writing
folder_names <- apply(expand.grid(c("A", "T", "C", "G"), c("A", "T", "C", "G"), c("A", "T", "C", "G")), 1, paste, collapse = "")
#folder_names <- list.dirs(directory_path, full.names = FALSE) # Get a list of folder names in the directory
...
for (folder_name in folder_names) {
  ...
  pos.peak=ecdf(df.plot[df.plot$status=="pos",]$dis)(40)
  neg.full.peak=ecdf(df.plot[df.plot$status=="neg.full",]$dis)(40)
  neg.indep.peak=ecdf(df.plot[df.plot$status=="neg.indep",]$dis)(40)
...
#R
directory_path <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer"
output_file <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer/CDF_fractions_50bp.txt"
output_connection <- file(output_file, "w") # Open the output file for writing
folder_names <- apply(expand.grid(c("A", "T", "C", "G"), c("A", "T", "C", "G"), c("A", "T", "C", "G")), 1, paste, collapse = "")
#folder_names <- list.dirs(directory_path, full.names = FALSE) # Get a list of folder names in the directory
...
for (folder_name in folder_names) {
  ...
  pos.peak=ecdf(df.plot[df.plot$status=="pos",]$dis)(50)
  neg.full.peak=ecdf(df.plot[df.plot$status=="neg.full",]$dis)(50)
  neg.indep.peak=ecdf(df.plot[df.plot$status=="neg.indep",]$dis)(50)
...
```{r engine='R', eval=TRUE, echo=TRUE}
CDF_df_16bp <- cbind(read.table("CDF_fractions.txt", header = FALSE, sep = "\t"), "16bp")
CDF_df_20bp <- cbind(read.table("CDF_fractions_20bp.txt", header = FALSE, sep = "\t"),"20bp")
CDF_df_30bp <- cbind(read.table("CDF_fractions_30bp.txt", header = FALSE, sep = "\t"),"30bp")
CDF_df_40bp <- cbind(read.table("CDF_fractions_40bp.txt", header = FALSE, sep = "\t"),"40bp")
CDF_df_50bp <- cbind(read.table("CDF_fractions_50bp.txt", header = FALSE, sep = "\t"),"50bp")

colnames(CDF_df_16bp)=c("triplet", "prox_enrich_pos", "prox_enrich_neg_full", "prox_enrich_neg_indep", "prox_enrich_ave_difference", "dis")
colnames(CDF_df_20bp)=c("triplet", "prox_enrich_pos", "prox_enrich_neg_full", "prox_enrich_neg_indep", "prox_enrich_ave_difference", "dis")
colnames(CDF_df_30bp)=c("triplet", "prox_enrich_pos", "prox_enrich_neg_full", "prox_enrich_neg_indep", "prox_enrich_ave_difference", "dis")
colnames(CDF_df_40bp)=c("triplet", "prox_enrich_pos", "prox_enrich_neg_full", "prox_enrich_neg_indep", "prox_enrich_ave_difference", "dis")
colnames(CDF_df_50bp)=c("triplet", "prox_enrich_pos", "prox_enrich_neg_full", "prox_enrich_neg_indep", "prox_enrich_ave_difference", "dis")
...
```{r engine='R', eval=TRUE, echo=TRUE}
library(dplyr)
CDF_df_pos<-rbind(CDF_df_16bp, CDF_df_20bp, CDF_df_30bp, CDF_df_40bp, CDF_df_50bp)
# Reorder levels of the factor variable based on descending fraction
CDF_df_pos <- CDF_df_pos %>%
  arrange(dis, desc(prox_enrich_ave_difference)) %>%
  group_by(dis) %>%
  mutate(triplet = factor(triplet, levels = triplet))

head(CDF_df_pos)
CDF_df_pos <- CDF_df_pos %>%
  mutate(my_color = ifelse(prox_enrich_ave_difference < 0, "grey",
                           ifelse(prox_enrich_ave_difference > 0.2, "lightblue", "lightgreen")))
head(CDF_df_pos)

pdf('3mer_fraction_of_Proximal_Emrichment_to_GATA3_peaks.pdf',width=8,height=8)
my.settings <- list(
  superpose.polygon=list(col=c("black", "grey"), border="transparent"),
  strip.background=list(col="grey80", cex = 0.6),
  strip.border=list(col="black")
...
print(
barchart(prox_enrich_ave_difference ~ triplet |dis,
          horizontal = FALSE,
          data=CDF_df_pos,
          #type="count",
          layout = c(1, 5),
          #ylim=c(-0,0.5),
          #col=ifelse(CDF_df_pos$prox_enrich_ave_difference >= 0, "skyblue", "grey"),
          #col = ifelse(CDF_df_pos$prox_enrich_ave_difference >= 0.20, "skyblue",
          #             ifelse(CDF_df_pos$prox_enrich_ave_difference >= 0 & CDF_df_pos$prox_enrich_ave_difference < 0.20, "lightgreen", "grey")),
          col=CDF_df_pos$my_color,
          origin = 0,
          scales = list(x = list(cex = 0.5, rot = 45)),
          xlab="3mer",
          ylab="fraction of proximal emrichment",
          main="3mer Proximal Emrichment to GATA3 peaks",
          cex.axis=0.5,
          between=list(y=0.5, x=0.5),
          font.axis=1,
          par.settings=my.settings,
...
It seems that the most robust categorization should be made at fraction differences==0. This will give us two category, "no enrichment" and "with enrichment". There are 32 3mers enriched in close proximity to GATA3 peaks (remains true when defining "close proximity" with distance ==16bp, 20bp, 30bp, 50bp). \
...
bwplot(prox_enrich_ave_difference ~ dis, 
       data=CDF_df_pos,
       #do.out = F,
       xlab='distance', ylab='fraction of proximal emrichment',
       ylim=c(-0.5, 1),
       par.settings = my.settings,
       scales=list(x=list(rot=45, cex=1)),
       panel = function(..., box.ratio) {
             panel.violin(...,col="white", varwidth = FALSE, box.ratio = box.ratio, do.out = F)
             panel.abline(h= 0, lty =2, col="red")
             panel.abline(h= 0.2, lty =2, col="red")
             #panel.bwplot(...)
...
It seems reasonable to put a cut-off at 0.2 and a cut-off at 0 to divide 3mers into three categories: enriched, neutral and depleted. \

**3mer in depleted category remains same while using different distance from summits** \
```{r engine='R', eval=TRUE, echo=TRUE}
CDF_df_16bp[CDF_df_16bp$prox_enrich_ave_difference < 0, ]$triplet
CDF_df_20bp[CDF_df_20bp$prox_enrich_ave_difference < 0, ]$triplet
CDF_df_30bp[CDF_df_30bp$prox_enrich_ave_difference < 0, ]$triplet
CDF_df_40bp[CDF_df_40bp$prox_enrich_ave_difference < 0, ]$triplet
CDF_df_50bp[CDF_df_50bp$prox_enrich_ave_difference < 0, ]$triplet
```
**3mer in enriched category changed while using different distance from summits** \
```{r engine='R', eval=TRUE, echo=TRUE}
CDF_df_16bp[CDF_df_16bp$prox_enrich_ave_difference > 0.2, ]$triplet # --10 3mers
CDF_df_20bp[CDF_df_20bp$prox_enrich_ave_difference > 0.2, ]$triplet # --10 3mers
CDF_df_30bp[CDF_df_30bp$prox_enrich_ave_difference > 0.2, ]$triplet # --8 3mers (TTT/AAA now in the neutral group)
CDF_df_40bp[CDF_df_40bp$prox_enrich_ave_difference > 0.2, ]$triplet # --8 3mers (TTT/AAA now in the neutral group)
CDF_df_50bp[CDF_df_50bp$prox_enrich_ave_difference > 0.2, ]$triplet # --4 3mers (TTT/AAA, AAT/ATT, GAT/ATC now in the neutral group)
```

It is unsurprising that, with an increase in the distance to 50bp, the enriched group exhibits fewer 3mers. As we expand the search radius for the closest 3mer (any 3mer), the likelihood of encountering random 3mers rises. This increase in random occurrences diminishes the observed 'enrichment' fraction. \

**3mer in neutral category changed while using different distance from summits** \
```{r engine='R',  eval=TRUE, echo=TRUE}
CDF_df_16bp[CDF_df_16bp$prox_enrich_ave_difference < 0.2 & CDF_df_50bp$prox_enrich_ave_difference > 0, ]$triplet # 22 3mers
CDF_df_20bp[CDF_df_20bp$prox_enrich_ave_difference < 0.2 & CDF_df_50bp$prox_enrich_ave_difference > 0, ]$triplet
CDF_df_30bp[CDF_df_30bp$prox_enrich_ave_difference < 0.2 & CDF_df_50bp$prox_enrich_ave_difference > 0, ]$triplet
CDF_df_40bp[CDF_df_40bp$prox_enrich_ave_difference < 0.2 & CDF_df_50bp$prox_enrich_ave_difference > 0, ]$triplet
CDF_df_50bp[CDF_df_50bp$prox_enrich_ave_difference < 0.2 & CDF_df_50bp$prox_enrich_ave_difference > 0, ]$triplet
```


```{r  out.width = "80%", out.height="100%", echo=F, fig.align = "center", fig.cap="3mer categorization"}
#library(knitr)
knitr::include_graphics("./3mer_fraction_of_Proximal_Emrichment_to_GATA3_peaks.png") 
```

**all traces in one plot** \
Based on the above analysis, we can cluster 3mer into () categories, namely "high enrichment", "low enrichment", and "no enrichment". And plot these traces in one single CDF plot. \

```{r engine='R', eval=F, echo=TRUE}
#module load R/4.1.2
#R
directory_path <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer"
output_file <- "/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/other_3mer/all_1st_3mer_distance.csv"
output_connection <- file(output_file, "w") # Open the output file for writing
folder_names <- apply(expand.grid(c("A", "T", "C", "G"), c("A", "T", "C", "G"), c("A", "T", "C", "G")), 1, paste, collapse = "")
#folder_names <- list.dirs(directory_path, full.names = FALSE) # Get a list of folder names in the directory
...
for (folder_name in folder_names) {
  ...
  # Initialize an empty data.table to store the combined data for the current folder
  combined_data <- data.table()
  
  # Read the three .bed files
  bed_files <- c(
    paste0("closest.1st", folder_name, ".to.indep.DHS.control.consensus.bed"),
    paste0("closest.1st", folder_name, ".to.full.DHS.control.consensus.bed"),
    paste0("closest.1st", folder_name, ".to.pos.control.motif.all.bed")
  )
  
  for (bed_file in bed_files) {
    # Read the .bed file into a data.table (adjust column indices accordingly)
    current_data <- fread(file.path(current_folder, bed_file), header = FALSE)
    
    # Extract the needed column's data
    status <- gsub("closest\\.1st.*to\\.(.*?)\\.bed", "\\1", bed_file)
    extracted_data <- current_data[, .(Column1 = V10, Column2 = V11, status)]  
    
    # Append the extracted data to the combined_data data.table
    combined_data <- rbind(combined_data, extracted_data)
  }
  
  # Perform any additional operations on combined_data if needed
  
  # Save the combined_data to the output file
  write.table(combined_data, file = output_file, append = TRUE, col.names = FALSE, row.names = FALSE, sep = "\t")
}
...
CDF plot with all traces (64 pos+64 neg) \
```{r engine='R', eval=F, echo=TRUE}
library(lattice)
library(latticeExtra)

all_1st_3mer_distance=read.table("all_1st_3mer_distance.csv", header=FALSE)
colnames(all_1st_3mer_distance)=c("triplet", "dis", "status")
all_1st_3mer_distance$supp=paste0(all_1st_3mer_distance$triplet, "_", all_1st_3mer_distance$status)
df.plot=rbind(all_1st_3mer_distance[all_1st_3mer_distance$status=="indep.DHS.control.consensus",], all_1st_3mer_distance[all_1st_3mer_distance$status=="pos.control.motif.all",])


df.plot$supp = factor(df.plot$supp, levels = c("ATA_pos.control.motif.all", "TAT_pos.control.motif.all", "TAA_pos.control.motif.all", "TTA_pos.control.motif.all", "ATC_pos.control.motif.all",
"GAT_pos.control.motif.all", "AAT_pos.control.motif.all", "ATT_pos.control.motif.all", "TTT_pos.control.motif.all", "AAA_pos.control.motif.all",
 "AGA_pos.control.motif.all", "TCT_pos.control.motif.all", "TAG_pos.control.motif.all", "CTA_pos.control.motif.all", "ATG_pos.control.motif.all",
"CAT_pos.control.motif.all", "TGA_pos.control.motif.all", "TCA_pos.control.motif.all", "GTA_pos.control.motif.all", "TAC_pos.control.motif.all",
"CTT_pos.control.motif.all", "AAG_pos.control.motif.all", "CAA_pos.control.motif.all", "TTG_pos.control.motif.all", "AAC_pos.control.motif.all",
"GTT_pos.control.motif.all", "ACA_pos.control.motif.all", "TGT_pos.control.motif.all", "GAA_pos.control.motif.all", "TTC_pos.control.motif.all",
"ACT_pos.control.motif.all", "AGT_pos.control.motif.all", "CTG_pos.control.motif.all", "CAG_pos.control.motif.all", "CGA_pos.control.motif.all",
"TCG_pos.control.motif.all", "CGT_pos.control.motif.all", "ACG_pos.control.motif.all", "CTC_pos.control.motif.all", "GAG_pos.control.motif.all",
"GTC_pos.control.motif.all", "GAC_pos.control.motif.all", "GGT_pos.control.motif.all", "ACC_pos.control.motif.all", "GCA_pos.control.motif.all",
"TGC_pos.control.motif.all", "CAC_pos.control.motif.all", "GTG_pos.control.motif.all", "TCC_pos.control.motif.all", "GGA_pos.control.motif.all",
"TGG_pos.control.motif.all", "CCA_pos.control.motif.all", "AGC_pos.control.motif.all", "GCT_pos.control.motif.all", "CCT_pos.control.motif.all",
"AGG_pos.control.motif.all", "CCG_pos.control.motif.all", "CGG_pos.control.motif.all", "CGC_pos.control.motif.all", "GCG_pos.control.motif.all",
"CCC_pos.control.motif.all", "GGG_pos.control.motif.all", "GCC_pos.control.motif.all", "GGC_pos.control.motif.all","ATA_indep.DHS.control.consensus", "TAT_indep.DHS.control.consensus", "TAA_indep.DHS.control.consensus", "TTA_indep.DHS.control.consensus", "ATC_indep.DHS.control.consensus",
"GAT_indep.DHS.control.consensus", "AAT_indep.DHS.control.consensus", "ATT_indep.DHS.control.consensus", "TTT_indep.DHS.control.consensus", "AAA_indep.DHS.control.consensus","AGA_indep.DHS.control.consensus", "TCT_indep.DHS.control.consensus", "TAG_indep.DHS.control.consensus", "CTA_indep.DHS.control.consensus", "ATG_indep.DHS.control.consensus","CAT_indep.DHS.control.consensus", "TGA_indep.DHS.control.consensus", "TCA_indep.DHS.control.consensus", "GTA_indep.DHS.control.consensus", "TAC_indep.DHS.control.consensus", "CTT_indep.DHS.control.consensus", "AAG_indep.DHS.control.consensus", "CAA_indep.DHS.control.consensus", "TTG_indep.DHS.control.consensus", "AAC_indep.DHS.control.consensus", "GTT_indep.DHS.control.consensus", "ACA_indep.DHS.control.consensus", "TGT_indep.DHS.control.consensus", "GAA_indep.DHS.control.consensus", "TTC_indep.DHS.control.consensus", "ACT_indep.DHS.control.consensus", "AGT_indep.DHS.control.consensus", "CTG_indep.DHS.control.consensus", "CAG_indep.DHS.control.consensus", "CGA_indep.DHS.control.consensus", "TCG_indep.DHS.control.consensus", "CGT_indep.DHS.control.consensus", "ACG_indep.DHS.control.consensus", "CTC_indep.DHS.control.consensus", "GAG_indep.DHS.control.consensus", "GTC_indep.DHS.control.consensus", "GAC_indep.DHS.control.consensus", "GGT_indep.DHS.control.consensus", "ACC_indep.DHS.control.consensus", "GCA_indep.DHS.control.consensus", "TGC_indep.DHS.control.consensus", "CAC_indep.DHS.control.consensus", "GTG_indep.DHS.control.consensus", "TCC_indep.DHS.control.consensus", "GGA_indep.DHS.control.consensus", "TGG_indep.DHS.control.consensus", "CCA_indep.DHS.control.consensus", "AGC_indep.DHS.control.consensus", "GCT_indep.DHS.control.consensus", "CCT_indep.DHS.control.consensus", "AGG_indep.DHS.control.consensus", "CCG_indep.DHS.control.consensus", "CGG_indep.DHS.control.consensus", "CGC_indep.DHS.control.consensus", "GCG_indep.DHS.control.consensus", "CCC_indep.DHS.control.consensus", "GGG_indep.DHS.control.consensus", "GCC_indep.DHS.control.consensus", "GGC_indep.DHS.control.consensus"))

pdf('3mer_Distance_from_peak_summit_all_traces.pdf',width=8,height=8)
print(ecdfplot(~log(abs(dis), base = 10), groups = supp, data = df.plot,
         auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
         col=c(colorRampPalette(c("red","pink"))(32),colorRampPalette(c("blue","lightblue"))(32), colorRampPalette(c("grey22","lightgrey"))(64)),
         aspect = 1,
        #xlim = c(0, 50000),
         scales=list(relation="free",alternating=c(1,1,1,1)),
         ylab = 'Cumulative Distribution Function',
         ...
         lwd=2,
         lty=c(1),
         par.settings = list(superpose.line = list(col=c(colorRampPalette(c("red","pink"))(32),colorRampPalette(c("blue","lightblue"))(32), colorRampPalette(c("grey22","lightgrey"))(64)), lwd=3), strip.background=list(col="grey85"))
         #panel = function(...) {
             #panel.abline(v= 1, lty =2, col="red")
             #panel.abline(v= 18, lty =2, col="grey30")
             #panel.ecdfplot(...)
...
This CDF looks too crowded. I will subset to "No enrichment" and "with enrichment" 3mers, then make two CDF plots. \
...
with_enrichment_list <- c("ATA", "TAT", "TAA", "TTA", "ATC", "GAT", "AAT", "ATT", "TTT", "AAA", "AGA", "TCT", "TAG", "CTA", "ATG", "CAT", "TGA", "TCA", "GTA", "TAC", "CTT", "AAG", "CAA", "TTG", "AAC", "GTT", "ACA", "TGT", "GAA", "TTC", "ACT","AGT")
no_enrichment_list <- c( "CTG", "CAG", "CGA", "TCG", "CGT", "ACG", "CTC", "GAG", "GTC", "GAC", "GGT", "ACC", "GCA", "TGC", "CAC", "GTG", "TCC", "GGA", "TGG", "CCA", "AGC", "GCT", "CCT","AGG", "CCG", "CGG", "CGC", "GCG", "CCC", "GGG", "GCC", "GGC")

df_plot_with_enr=df.plot[df.plot$triplet %in% with_enrichment_list, ]
df_plot_no_enr=df.plot[df.plot$triplet %in% no_enrichment_list, ]



df_plot_with_enr$supp = factor(df_plot_with_enr$supp, levels = c("ATA_pos.control.motif.all", "TAT_pos.control.motif.all", "TAA_pos.control.motif.all", "TTA_pos.control.motif.all", "ATC_pos.control.motif.all",
"GAT_pos.control.motif.all", "AAT_pos.control.motif.all", "ATT_pos.control.motif.all", "TTT_pos.control.motif.all", "AAA_pos.control.motif.all",
 "AGA_pos.control.motif.all", "TCT_pos.control.motif.all", "TAG_pos.control.motif.all", "CTA_pos.control.motif.all", "ATG_pos.control.motif.all",
"CAT_pos.control.motif.all", "TGA_pos.control.motif.all", "TCA_pos.control.motif.all", "GTA_pos.control.motif.all", "TAC_pos.control.motif.all",
"CTT_pos.control.motif.all", "AAG_pos.control.motif.all", "CAA_pos.control.motif.all", "TTG_pos.control.motif.all", "AAC_pos.control.motif.all",
"GTT_pos.control.motif.all", "ACA_pos.control.motif.all", "TGT_pos.control.motif.all", "GAA_pos.control.motif.all", "TTC_pos.control.motif.all",
"ACT_pos.control.motif.all", "AGT_pos.control.motif.all",
"ATA_indep.DHS.control.consensus", "TAT_indep.DHS.control.consensus", "TAA_indep.DHS.control.consensus", "TTA_indep.DHS.control.consensus", "ATC_indep.DHS.control.consensus", "GAT_indep.DHS.control.consensus", "AAT_indep.DHS.control.consensus", "ATT_indep.DHS.control.consensus", "TTT_indep.DHS.control.consensus", "AAA_indep.DHS.control.consensus","AGA_indep.DHS.control.consensus", "TCT_indep.DHS.control.consensus", "TAG_indep.DHS.control.consensus", "CTA_indep.DHS.control.consensus", "ATG_indep.DHS.control.consensus","CAT_indep.DHS.control.consensus", "TGA_indep.DHS.control.consensus", "TCA_indep.DHS.control.consensus", "GTA_indep.DHS.control.consensus", "TAC_indep.DHS.control.consensus", "CTT_indep.DHS.control.consensus", "AAG_indep.DHS.control.consensus", "CAA_indep.DHS.control.consensus", "TTG_indep.DHS.control.consensus", "AAC_indep.DHS.control.consensus", "GTT_indep.DHS.control.consensus", "ACA_indep.DHS.control.consensus", "TGT_indep.DHS.control.consensus", "GAA_indep.DHS.control.consensus", "TTC_indep.DHS.control.consensus", "ACT_indep.DHS.control.consensus","AGT_indep.DHS.control.consensus"))
df_plot_no_enr$supp = factor(df_plot_no_enr$supp, levels = c("CTG_pos.control.motif.all", "CAG_pos.control.motif.all", "CGA_pos.control.motif.all",
"TCG_pos.control.motif.all", "CGT_pos.control.motif.all", "ACG_pos.control.motif.all", "CTC_pos.control.motif.all", "GAG_pos.control.motif.all",
"GTC_pos.control.motif.all", "GAC_pos.control.motif.all", "GGT_pos.control.motif.all", "ACC_pos.control.motif.all", "GCA_pos.control.motif.all",
"TGC_pos.control.motif.all", "CAC_pos.control.motif.all", "GTG_pos.control.motif.all", "TCC_pos.control.motif.all", "GGA_pos.control.motif.all",
"TGG_pos.control.motif.all", "CCA_pos.control.motif.all", "AGC_pos.control.motif.all", "GCT_pos.control.motif.all", "CCT_pos.control.motif.all",
"AGG_pos.control.motif.all", "CCG_pos.control.motif.all", "CGG_pos.control.motif.all", "CGC_pos.control.motif.all", "GCG_pos.control.motif.all",
"CCC_pos.control.motif.all", "GGG_pos.control.motif.all", "GCC_pos.control.motif.all", "GGC_pos.control.motif.all", "CTG_indep.DHS.control.consensus", "CAG_indep.DHS.control.consensus", "CGA_indep.DHS.control.consensus", "TCG_indep.DHS.control.consensus", "CGT_indep.DHS.control.consensus", "ACG_indep.DHS.control.consensus", "CTC_indep.DHS.control.consensus", "GAG_indep.DHS.control.consensus", "GTC_indep.DHS.control.consensus", "GAC_indep.DHS.control.consensus", "GGT_indep.DHS.control.consensus", "ACC_indep.DHS.control.consensus", "GCA_indep.DHS.control.consensus", "TGC_indep.DHS.control.consensus", "CAC_indep.DHS.control.consensus", "GTG_indep.DHS.control.consensus", "TCC_indep.DHS.control.consensus", "GGA_indep.DHS.control.consensus", "TGG_indep.DHS.control.consensus", "CCA_indep.DHS.control.consensus", "AGC_indep.DHS.control.consensus", "GCT_indep.DHS.control.consensus", "CCT_indep.DHS.control.consensus", "AGG_indep.DHS.control.consensus", "CCG_indep.DHS.control.consensus", "CGG_indep.DHS.control.consensus", "CGC_indep.DHS.control.consensus", "GCG_indep.DHS.control.consensus", "CCC_indep.DHS.control.consensus", "GGG_indep.DHS.control.consensus", "GCC_indep.DHS.control.consensus", "GGC_indep.DHS.control.consensus"))

pdf('3mer_Distance_from_peak_summit_with_enr_traces.pdf',width=8,height=15)
print(ecdfplot(~log(abs(dis), base = 10), groups = supp, data = df_plot_with_enr,
         auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
         col=c(colorRampPalette(c("red","pink"))(32), colorRampPalette(c("grey22","lightgrey"))(32)),
         aspect = 1,
        #xlim = c(0, 50000),
         scales=list(relation="free",alternating=c(1,1,1,1)),
         ylab = 'Cumulative Distribution Function',
         ...
         lwd=2,
         lty=c(1),
         par.settings = list(superpose.line = list(col=c(colorRampPalette(c("red","pink"))(32), colorRampPalette(c("grey22","lightgrey"))(32)), lwd=3), strip.background=list(col="grey85"))
         #panel = function(...) {
             #panel.abline(v= 1, lty =2, col="red")
             #panel.abline(v= 18, lty =2, col="grey30")
             #panel.ecdfplot(...)
...
pdf('3mer_Distance_from_peak_summit_no_enr_traces.pdf',width=8,height=15)
print(ecdfplot(~log(abs(dis), base = 10), groups = supp, data = df_plot_no_enr,
         auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
         col=c(colorRampPalette(c("blue","lightblue"))(32), colorRampPalette(c("grey22","lightgrey"))(32)),
         aspect = 1,
        #xlim = c(0, 50000),
         scales=list(relation="free",alternating=c(1,1,1,1)),
         ylab = 'Cumulative Distribution Function',
         ...
         lwd=2,
         lty=c(1),
         par.settings = list(superpose.line = list(col=c(colorRampPalette(c("blue","lightblue"))(32), colorRampPalette(c("grey22","lightgrey"))(32)), lwd=3), strip.background=list(col="grey85"))
         #panel = function(...) {
             #panel.abline(v= 1, lty =2, col="red")
             #panel.abline(v= 18, lty =2, col="grey30")
             #panel.ecdfplot(...)
...
We could further separate the 3mers into "enriched" and "neutral" groups.  \
And we could make 5 CDFs for the five different distance from summits to visualize 3mer change within each category. \
...
df_plot_with_enr=df.plot[df.plot$triplet %in% with_enrichment_list, ]
df_plot_with_enr$supp = factor(df_plot_with_enr$supp, levels = c( "ATA_indep.DHS.control.consensus", "TAT_indep.DHS.control.consensus", "TAA_indep.DHS.control.consensus", "TTA_indep.DHS.control.consensus", "ATC_indep.DHS.control.consensus", "GAT_indep.DHS.control.consensus", "AAT_indep.DHS.control.consensus", "ATT_indep.DHS.control.consensus", "TTT_indep.DHS.control.consensus", "AAA_indep.DHS.control.consensus","AGA_indep.DHS.control.consensus", "TCT_indep.DHS.control.consensus", "TAG_indep.DHS.control.consensus", "CTA_indep.DHS.control.consensus", "ATG_indep.DHS.control.consensus","CAT_indep.DHS.control.consensus", "TGA_indep.DHS.control.consensus", "TCA_indep.DHS.control.consensus", "GTA_indep.DHS.control.consensus", "TAC_indep.DHS.control.consensus", "CTT_indep.DHS.control.consensus", "AAG_indep.DHS.control.consensus", "CAA_indep.DHS.control.consensus", "TTG_indep.DHS.control.consensus", "AAC_indep.DHS.control.consensus", "GTT_indep.DHS.control.consensus", "ACA_indep.DHS.control.consensus", "TGT_indep.DHS.control.consensus", "GAA_indep.DHS.control.consensus", "TTC_indep.DHS.control.consensus", "ACT_indep.DHS.control.consensus","AGT_indep.DHS.control.consensus", "ATA_pos.control.motif.all", "TAT_pos.control.motif.all", "TAA_pos.control.motif.all", "TTA_pos.control.motif.all", "ATC_pos.control.motif.all", "GAT_pos.control.motif.all", "AAT_pos.control.motif.all", "ATT_pos.control.motif.all", "TTT_pos.control.motif.all", "AAA_pos.control.motif.all", "AGA_pos.control.motif.all", "TCT_pos.control.motif.all", "TAG_pos.control.motif.all", "CTA_pos.control.motif.all", "ATG_pos.control.motif.all", "CAT_pos.control.motif.all", "TGA_pos.control.motif.all", "TCA_pos.control.motif.all", "GTA_pos.control.motif.all", "TAC_pos.control.motif.all", "CTT_pos.control.motif.all", "AAG_pos.control.motif.all", "CAA_pos.control.motif.all", "TTG_pos.control.motif.all", "AAC_pos.control.motif.all", "GTT_pos.control.motif.all", "ACA_pos.control.motif.all", "TGT_pos.control.motif.all", "GAA_pos.control.motif.all", "TTC_pos.control.motif.all", "ACT_pos.control.motif.all", "AGT_pos.control.motif.all"))

pdf('3mer_Distance_from_peak_summit_with_enr_traces_in_group.pdf',width=8,height=15)
print(ecdfplot(~log(abs(dis), base = 10), groups = supp, data = df_plot_with_enr,
         auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
         col=c(colorRampPalette(c("grey22","lightgrey"))(32), "red","red","red","red", "hotpink","hotpink","hotpink","hotpink","pink","pink", colorRampPalette(c("green","lightgreen"))(22)),
         aspect = 1,
        #xlim = c(0, 50000),
         scales=list(relation="free",alternating=c(1,1,1,1)),
         ylab = 'Cumulative Distribution Function',
         ...
         lwd=2,
         lty=c(1),
         par.settings = list(superpose.line = list(col=c(colorRampPalette(c("grey22","lightgrey"))(32), "red","red","red","red", "hotpink","hotpink","hotpink","hotpink","pink","pink", colorRampPalette(c("green","lightgreen"))(22)), lwd=3), strip.background=list(col="grey85"))
         #panel = function(...) {
             #panel.abline(v= 1, lty =2, col="red")
             #panel.abline(v= 18, lty =2, col="grey30")
             #panel.ecdfplot(...)
...
### Conclusion
There are 32 3mers ( with GAT/ATC, ATA/TAT, TAA/TTA on the top of the list) exhibit closer proximity to the GATA3 peak summit and demonstrate enrichment compared to DHS regions near (use distance==16bp) the GATA3 peak summit. \
enriched (n=10): "AAA" "TAA" "ATA" "TTA" "AAT" "TAT" "GAT" "ATT" "TTT" "ATC" \
neutral (n=22): "CAA" "GAA" "CTA" "GTA" "ACA" "TCA" "AGA" "TGA" "CAT" "CTT" "GTT" "ACT" "TCT" "AGT" "TGT" "AAC" "TAC" "TTC" "AAG" "TAG" "ATG" "TTG" \

There are 32 3mers do not show closer proximity and have no enrichment near GATA3 peaks. These 3mers shows a negative CDF differences in the ranked list. These 3mers are: GGC/GCC, GGG/CCC, GCG/CGC, CGG/CCG, AGG/CCT, GCT/AGC, CCA/TGG, GGA/TCC, GTG/CAC, TGC/GCA, ACC/GGT, GAT/GTC, GAG/CTC, ACG/CGT, TCG/CGA, CAG/CTG. We can put these 3mers aside for now. \
depleted:  "CTG", "CAG", "CGA", "TCG", "CGT", "ACG", "CTC", "GAG", "GTC", "GAC", "GGT", "ACC", "GCA", "TGC", "CAC", "GTG", "TCC", "GGA", "TGG", "CCA", "AGC", "GCT", "CCT","AGG", "CCG", "CGG", "CGC", "GCG", "CCC", "GGG", "GCC", “GGC" \

**The different 3mer in enriched cluster is AAA/TTT while we change the 16/20bp to 30/40bp. I want to include AAA/TTT since they are reasonable 3mer while looking at the PWM of GATA3 motifs.** \
...
**Plan**: Use the positive control GATA3 peaks with known motif structure: \
First, identify the closest GAT to the peak summit on the plus and minus strands, respectively. \
For the plus strand: \
a. Anchor at the 1st +GAT and find the 2nd closest GAT on the same strand (+). This is equivalent to searching for the opposite strand ATC (coherence check1). \
b. Anchor at the 1st +GAT and find the 2nd closest GAT on the opposite strand (-). This is equivalent to searching for the same strand ATC (coherence check2). \


For the minus strand: \
a. Anchor at the 1st -GAT and find the 2nd closest GAT on the same strand (-). This is equivalent to searching for the opposite strand ATC (coherence check3). \
b. Anchor at the 1st -GAT and find the 2nd closest GAT on the opposite strand (+). This is equivalent to searching for the same strand ATC (coherence check4). \


Coherence check5: Visualize on genome browser \

### closest GAT on + strand or - strand

There are 8 motifs found by MEME/STREME in GATA3 peaks. Among these 8 motifs, 5 of them have specific motif structure that we are interested of (two GATA3 motif half sites with un-fixed spacing in between). These are motif 1,2,4,5,6. \
```{r  out.width = "80%", out.height="80%", echo=F, fig.align = "center", fig.cap="GATA3 ChIP peak MME/STREME found motifs"}
#library(knitr)
knitr::include_graphics("./GATA3_ChIP_motifs.png") 
```

In previous analysis (refer to section 2.7.3 positive control: 1 peak with 1 best motif), we have made specific BED files contain information of GATA3 peaks with its best motifs (five motifs with known structure). \

```{r engine='bash',  class.source = "revision", eval=F, echo=TRUE}
dir=/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/GAT_3mer_analaysis/MAST_positive_control/

wc -l ${dir}*GATA3_peak_161win_with_motif*.bed
 12470 GATA3_peak_161win_with_motif_1.bed
  11475 GATA3_peak_161win_with_motif_2.bed
   6505 GATA3_peak_161win_with_motif_4.bed
   4167 GATA3_peak_161win_with_motif_5.bed
...
We will find the closest GAT on + strand or on - strand for peak with motif1, motif4, motif2, motif6, respectively. \
...
plus.GAT.file=read.table(file = paste0(dir1,"hg38.3.3.3plus.36_GAT.bed"), sep="\t", header=FALSE)
minus.GAT.file=read.table(file = paste0(dir1,"hg38.3.3.3minus.14_GAT.bed"), sep="\t", header=FALSE)
head(plus.GAT.file)
nrow(plus.GAT.file)
tail(minus.GAT.file)
nrow(minus.GAT.file)
...
GATA3_peak_summits_motif1=center.bed(read.table(paste0(dir2, "GATA3_peak_161win_with_motif_1.bed"), header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
GATA3_peak_summits_motif4=center.bed(read.table(paste0(dir2, "GATA3_peak_161win_with_motif_4.bed"), header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
GATA3_peak_summits_motif2=center.bed(read.table(paste0(dir2, "GATA3_peak_161win_with_motif_2.bed"), header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
GATA3_peak_summits_motif6=center.bed(read.table(paste0(dir2, "GATA3_peak_161win_with_motif_6.bed"), header=FALSE), upstreamWindow = 0, downstreamWindow = 0)

GATA3_peak_summits_motif5=center.bed(read.table(paste0(dir2, "GATA3_peak_161win_with_motif_5.bed"), header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
...
closest.1st.plus.GAT.to.GATA3.with.motif1=bedTools.closest(bed1 = GATA3_peak_summits_motif1[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')
closest.1st.plus.GAT.to.GATA3.with.motif4=bedTools.closest(bed1 = GATA3_peak_summits_motif4[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')
closest.1st.plus.GAT.to.GATA3.with.motif2=bedTools.closest(bed1 = GATA3_peak_summits_motif2[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')
closest.1st.plus.GAT.to.GATA3.with.motif6=bedTools.closest(bed1 = GATA3_peak_summits_motif6[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')

closest.1st.plus.GAT.to.GATA3.with.motif5=bedTools.closest(bed1 = GATA3_peak_summits_motif5[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')



write.table(closest.1st.plus.GAT.to.GATA3.with.motif1,file= 'closest.1st.plus.GAT.to.GATA3.with.motif1.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.1st.plus.GAT.to.GATA3.with.motif4,file= 'closest.1st.plus.GAT.to.GATA3.with.motif4.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.1st.plus.GAT.to.GATA3.with.motif2,file= 'closest.1st.plus.GAT.to.GATA3.with.motif2.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.1st.plus.GAT.to.GATA3.with.motif6,file= 'closest.1st.plus.GAT.to.GATA3.with.motif6.bed', quote=F,sep="\t",col.names=F,row.names=F)

write.table(closest.1st.plus.GAT.to.GATA3.with.motif5,file= 'closest.1st.plus.GAT.to.GATA3.with.motif5.bed', quote=F,sep="\t",col.names=F,row.names=F)


closest.1st.plus.GAT.to.indep.DHS.control.consensus=bedTools.closest(bed1 = indep.DHS.control.consensus[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')
write.table(closest.1st.plus.GAT.to.indep.DHS.control.consensus,file= 'closest.1st.plus.GAT.to.indep.DHS.control.consensus.bed', quote=F,sep="\t",col.names=F,row.names=F)


# closestBed--1st closest minus

closest.1st.minus.GAT.to.GATA3.with.motif1=bedTools.closest(bed1 = GATA3_peak_summits_motif1[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')
closest.1st.minus.GAT.to.GATA3.with.motif4=bedTools.closest(bed1 = GATA3_peak_summits_motif4[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')
closest.1st.minus.GAT.to.GATA3.with.motif2=bedTools.closest(bed1 = GATA3_peak_summits_motif2[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')
closest.1st.minus.GAT.to.GATA3.with.motif6=bedTools.closest(bed1 = GATA3_peak_summits_motif6[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')

closest.1st.minus.GAT.to.GATA3.with.motif5=bedTools.closest(bed1 = GATA3_peak_summits_motif5[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')



write.table(closest.1st.minus.GAT.to.GATA3.with.motif1,file= 'closest.1st.minus.GAT.to.GATA3.with.motif1.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.1st.minus.GAT.to.GATA3.with.motif4,file= 'closest.1st.minus.GAT.to.GATA3.with.motif4.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.1st.minus.GAT.to.GATA3.with.motif2,file= 'closest.1st.minus.GAT.to.GATA3.with.motif2.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.1st.minus.GAT.to.GATA3.with.motif6,file= 'closest.1st.minus.GAT.to.GATA3.with.motif6.bed', quote=F,sep="\t",col.names=F,row.names=F)

write.table(closest.1st.minus.GAT.to.GATA3.with.motif5,file= 'closest.1st.minus.GAT.to.GATA3.with.motif5.bed', quote=F,sep="\t",col.names=F,row.names=F)


closest.1st.minus.GAT.to.indep.DHS.control.consensus=bedTools.closest(bed1 = indep.DHS.control.consensus[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')
write.table(closest.1st.minus.GAT.to.indep.DHS.control.consensus,file= 'closest.1st.minus.GAT.to.indep.DHS.control.consensus.bed', quote=F,sep="\t",col.names=F,row.names=F)


save.image('240125_GATA3_ChIP_clsestbed.Rdata') 
```
...
Now we have the : \
closest.1st.plus.GAT.to.GATA3.with.motif1.bed \
closest.1st.minus.GAT.to.GATA3.with.motif1.bed \

We want to extract the GAT coordinates info from the BED files, and remove the overlapped regions from the files that contains GAT coordinates info on the entire hg38 genome (hg38.3.3.3plus.36_GAT.bed/hg38.3.3.3minus.14_GAT.bed). Then locate the "second closest" + or - GAT to +/- "closest" GAT. \

Then draw PDF to inspect the enrichment. \
...
module load bedtools 
dir=/labs/Guertin/siyu/Sathyan_GATA3_ChIP_pool1_pool2/overrep_3mer/hg38_full_kmer3_rs1000/seqdump/
sort -k1,1 -k2,2n ${dir}hg38.3.3.3plus.36_GAT.bed > hg38.3.3.3plus.36_GAT.sorted.bed
awk '{print $4, $5, $6, $7, $8, $9, $10}' closest.1st.plus.GAT.to.GATA3.with.motif1.bed | awk '{$1=$1}1' OFS="\t" | uniq | sort -k1,1 -k2,2n > closest.1st.plus.GAT.to.GATA3.with.motif1.uniq.sorted.bed
bedtools subtract -a hg38.3.3.3plus.36_GAT.sorted.bed -b closest.1st.plus.GAT.to.GATA3.with.motif1.uniq.sorted.bed -f 1.00 -s > hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_motif1.bed

wc -l hg38.3.3.3plus.36_GAT.sorted.bed #39090736
wc -l closest.1st.plus.GAT.to.GATA3.with.motif1.uniq.sorted.bed #12461
wc -l hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_motif1.bed #39078275
head -5 hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_motif1.bed
#chr1	11159	11162	36	36	+	GAT
#chr1	11462	11465	36	36	+	GAT
#chr1	11521	11524	36	36	+	GAT
#chr1	11569	11572	36	36	+	GAT
#chr1	11651	11654	36	36	+	GAT


sort -k1,1 -k2,2n ${dir}hg38.3.3.3minus.14_GAT.bed > hg38.3.3.3minus.14_GAT.sorted.bed
awk '{print $4, $5, $6, $7, $8, $9, $10}' closest.1st.minus.GAT.to.GATA3.with.motif1.bed | awk '{$1=$1}1' OFS="\t" | uniq | sort -k1,1 -k2,2n > closest.1st.minus.GAT.to.GATA3.with.motif1.uniq.sorted.bed
bedtools subtract -a hg38.3.3.3minus.14_GAT.sorted.bed -b closest.1st.minus.GAT.to.GATA3.with.motif1.uniq.sorted.bed -f 1.00 -s > hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_GATA3_with_motif1.bed

wc -l hg38.3.3.3minus.14_GAT.sorted.bed #38717592
wc -l closest.1st.minus.GAT.to.GATA3.with.motif1.uniq.sorted.bed #12460
wc -l hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_GATA3_with_motif1.bed #38705132
#38717592-12460
#[1] 38705132

head -5 hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_GATA3_with_motif1.bed
#chr1	10545	10548	14	14	-	GAT
#chr1	11145	11148	14	14	-	GAT
#chr1	11160	11163	14	14	-	GAT
#chr1	11576	11579	14	14	-	GAT
...
**Find closest 2nd GAT to 1st GAT** \
find the + or - 2nd GAT to the closest +GAT \
find the + or - 2nd GAT to the closest -GAT \
...
library(bigWig)
# GAT coordinates without overlapped closest GAT -- relative to G for plus, relative to C for minus
plus.GAT.file=fiveprime.bed(read.table(file = "hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_motif1.bed", sep="\t", header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
minus.GAT.file=fiveprime.bed(read.table(file = "hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_GATA3_with_motif1.bed", sep="\t", header=FALSE), upstreamWindow = -1, downstreamWindow = 1)

head(plus.GAT.file) # find coordinates at Genome Browser to make sure the anchor point is correct -- anchor at G on GAT
#  V1    V2    V3 V4 V5 V6  V7
#1 chr1 11159 11160 36 36  + GAT
#2 chr1 11462 11463 36 36  + GAT
#3 chr1 11521 11522 36 36  + GAT
#4 chr1 11569 11570 36 36  + GAT
#5 chr1 11651 11652 36 36  + GAT
#6 chr1 11684 11685 36 36  + GAT


nrow(plus.GAT.file)
#[1] 39078275

head(minus.GAT.file) # find coordinates at Genome Browser to make sure the anchor point is correct -- anchor at C on ATC
# V1    V2    V3 V4 V5 V6  V7
#1 chr1 10547 10548 14 14  - GAT
#2 chr1 11147 11148 14 14  - GAT
#3 chr1 11162 11163 14 14  - GAT
#4 chr1 11578 11579 14 14  - GAT
#5 chr1 11599 11600 14 14  - GAT
#6 chr1 11848 11849 14 14  - GAT

nrow(minus.GAT.file)
#[1] 38705132

# anchor position: closest +/- GAT relative to G
closest_plus_GAT_to_GATA3_peak_summits_motif1=fiveprime.bed(read.table("closest.1st.plus.GAT.to.GATA3.with.motif1.uniq.sorted.bed", header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
closest_minus_GAT_to_GATA3_peak_summits_motif1=fiveprime.bed(read.table("closest.1st.minus.GAT.to.GATA3.with.motif1.uniq.sorted.bed", header=FALSE), upstreamWindow = -1, downstreamWindow = 1)

nrow(closest_plus_GAT_to_GATA3_peak_summits_motif1)
#[1] 12461
head(closest_plus_GAT_to_GATA3_peak_summits_motif1) # find coordinates at Genome Browser to make sure the anchor point is correct -- anchor at G on GAT
# V1      V2      V3 V4 V5 V6  V7
#1 chr1 1080566 1080567 36 36  + GAT
#2 chr1 1238219 1238220 36 36  + GAT
#3 chr1 1686429 1686430 36 36  + GAT
#4 chr1 1741128 1741129 36 36  + GAT
#5 chr1 2002016 2002017 36 36  + GAT
#6 chr1 3127380 3127381 36 36  + GAT

nrow(closest_minus_GAT_to_GATA3_peak_summits_motif1)
#[1] 12460
head(closest_minus_GAT_to_GATA3_peak_summits_motif1) # find coordinates at Genome Browser to make sure the anchor point is correct -- anchor at C on ATC
#    V1      V2      V3 V4 V5 V6  V7
#1 chr1 1080574 1080575 14 14  - GAT
#2 chr1 1238227 1238228 14 14  - GAT
#3 chr1 1686413 1686414 14 14  - GAT
#4 chr1 1741178 1741179 14 14  - GAT
#5 chr1 2002024 2002025 14 14  - GAT
#6 chr1 3127369 3127370 14 14  - GAT


# 2nd closest +/-GAT to closest plus GAT

closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif1=bedTools.closest(bed1 = closest_plus_GAT_to_GATA3_peak_summits_motif1[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')
closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif1=bedTools.closest(bed1 = closest_minus_GAT_to_GATA3_peak_summits_motif1[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')



write.table(closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif1,file= 'closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif1.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif1,file= 'closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif1.bed', quote=F,sep="\t",col.names=F,row.names=F)

# 2nd closest +/- GAT to closest minus GAT
closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif1=bedTools.closest(bed1 = closest_plus_GAT_to_GATA3_peak_summits_motif1[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')
closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif1=bedTools.closest(bed1 = closest_minus_GAT_to_GATA3_peak_summits_motif1[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')



write.table(closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif1,file= 'closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif1.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif1,file= 'closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif1.bed', quote=F,sep="\t",col.names=F,row.names=F)


save.image('240129_GATA3_ChIP_2nd_clsestbed.Rdata') 
```

### PDFs
```{r engine='R',  class.source = "revision", eval=F, echo=TRUE}
df.plot = data.frame(matrix(nrow = 0, ncol = 3))     
colnames(df.plot) = c("dis", "anchor_status", "query_status")
for (closest_2nd_dis in Sys.glob(file.path("./closest.2nd*GAT.to.1st*GAT.GATA3.motif1.bed"))) {
    print(closest_2nd_dis)
    anchor_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.*.to.1st.')[[1]][2]), ".GATA3.motif1.bed")[[1]][1]
    print(anchor_status)
    query_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.')[[1]][2]), ".to.1st.*.GATA3.motif1.bed")[[1]][1]
    print(query_status)
    temp = cbind(read.table(closest_2nd_dis, header=F, comment.char='')[,11], anchor_status, query_status)
    df.plot = rbind(df.plot,temp)
}
str(df.plot)
#'data.frame':	49842 obs. of  3 variables:
# $ V1           : chr  "240" "3" "117" "114" ...
# $ anchor_status: chr  "minus.GAT" "minus.GAT" "minus.GAT" "minus.GAT" ...
# $ query_status : chr  "minus.GAT" "minus.GAT" "minus.GAT" "minus.GAT" ...
...
colnames(df.plot) = c("dis", "anchor_status", "query_status")
head(df.plot)
# dis anchor_status query_status
#1 240     minus.GAT    minus.GAT
#2   3     minus.GAT    minus.GAT
#3 117     minus.GAT    minus.GAT
#4 114     minus.GAT    minus.GAT
#5  27     minus.GAT    minus.GAT
#6   3     minus.GAT    minus.GAT
nrow(df.plot)
#[1] 49842
...
library(latticeExtra)
  # PDF plot
  pdf('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.pdf', width=15,height=5)
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
            col=c("red","blue"),
            ...
            panel = function(x, ...) {
                                      panel.densityplot(x, ...)
                                      #panel.xyplot(x, y, col = "skyblue", pch = 16)
                                      panel.ablineq(v = 8, col = "red", lty = 2, adj=c(0,1), label="8")}
            
))
  dev.off()
  
    
   png('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.png')
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
...
```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="PDF plots of GATA3 peak with motif1: 3mer structure"}
library(knitr)
knitr::include_graphics("./PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.png") 
```

**adds individual datapoint on the density plot** \
```{r engine='R',  class.source = "revision", eval=F, echo=TRUE}
df.plot = data.frame(matrix(nrow = 0, ncol = 5))     
colnames(df.plot) = c("dis","anchor_status", "query_status","abs.dis", "density")
for (closest_2nd_dis in Sys.glob(file.path("./closest.2nd*GAT.to.1st*GAT.GATA3.motif1.bed"))) {
    print(closest_2nd_dis)
    anchor_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.*.to.1st.')[[1]][2]), ".GATA3.motif1.bed")[[1]][1]
    print(anchor_status)
    ...
    temp$dis=as.numeric(temp$dis)
    temp$abs.dis=abs(temp$dis)
    density_values = density(temp$abs.dis, from=0, to=50, bw="nrd0")
    temp$density =approx(density_values$x, density_values$y, temp$abs.dis)$y
    df.plot = rbind(df.plot,temp)
}

str(df.plot)
#'data.frame':	49842 obs. of  5 variables:
# $ dis          : int  240 3 117 114 27 3 5 11 67 39 ...
# $ anchor_status: chr  "minus.GAT" "minus.GAT" "minus.GAT" "minus.GAT" ...
# $ query_status : chr  "minus.GAT" "minus.GAT" "minus.GAT" "minus.GAT" ...
# $ abs.dis      : int  240 3 117 114 27 3 5 11 67 39 ...
# $ density      : num  NA 0.0165 NA NA 0.0138 ...
unique(df.plot$anchor_status)
#[1] "minus.GAT" "plus.GAT" 
unique(df.plot$query_status)
#[1] "minus.GAT" "plus.GAT" 
head(df.plot)
#  dis anchor_status query_status abs.dis    density
#1 240     minus.GAT    minus.GAT     240         NA
#2   3     minus.GAT    minus.GAT       3 0.01653378
#3 117     minus.GAT    minus.GAT     117         NA
#4 114     minus.GAT    minus.GAT     114         NA
#5  27     minus.GAT    minus.GAT      27 0.01379179
#6   3     minus.GAT    minus.GAT       3 0.01653378
nrow(df.plot)
#[1] 49842
...
library(lattice)
library(latticeExtra)
pdf('PDF2_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.pdf', width=15,height=5)
print(xyplot(density ~ abs.dis | anchor_status, 
         data = df.plot,
         groups = query_status,
         auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
         aspect = 1,
...
df.plot$anchor_status = factor(df.plot$anchor_status, levels = c("plus.GAT", "minus.GAT"))
df.plot$query_status = factor(df.plot$query_status, levels = c("plus.GAT", "minus.GAT"))
df.plot$dis=as.integer(df.plot$dis)

library(lattice)
library(latticeExtra)

 pdf('his_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.pdf', width=15,height=5)
 print(histogram( ~ abs(dis) | anchor_status,
                  groups = query_status,
                  auto.key = list(space = "right", main="2nd GAT",rectangles=FALSE, lines=TRUE, points=FALSE, cex = 1),
                  #key = list(space = "right", text=list(c("+ strand", "- strand"), cex=1)),
                  data = df.plot, 
                  aspect = 1,
                  breaks=seq(from=0,to=24921, by=1),
                  #breaks=1,
                  #nint=1,
                  type = "density",
                  xlim=c(0,50),
                  ...
                  main="GATA3 peak with motif1",
                  par.settings = list(superpose.line = list(col=c("red", "blue"), lwd=3), strip.background=list(col="grey85")),
                  panel = function(x,...) {#panel.histogram(...)
                                           #panel.densityplot(...)
                                          panel.densityplot(x, data = df.plot, 
                                                            ...
                                                            #type = "count",
                                                            col=c("red","blue"), ...)
                                         panel.superpose(x, ..., panel.groups=panel.histogram, 
                                  col=c('red', 'blue'), alpha=0.4)
         }))
  dev.off()
  
  png('his_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.png')
 print(histogram( ~ abs(dis) | anchor_status,
                  groups = query_status,
                  auto.key = list(space = "right", main="2nd GAT",rectangles=FALSE, lines=TRUE, points=FALSE, cex = 1),
                  #key = list(space = "right", text=list(c("+ strand", "- strand"), cex=1)),
...
```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="histogram of GATA3 peak with motif1: 3mer structure"}
#library(knitr)
knitr::include_graphics("./his_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.png") 
```

### xyplot
Confirm if the y value from lattice "type==density" is the relative frequency of each distance occurrence in data. \
```{r engine='R',  class.source = "revision", eval=F, echo=TRUE}
calculate_relative_frequency <- function(data) {
  # Use table() to create a frequency table
  relative_frequencies <- table(data)/length(data)
  result <- data.frame(value = names(relative_frequencies), rel_freq = as.vector(relative_frequencies))
  return(result)
}
...
# $ anchor_status: chr  "minus.GAT" "minus.GAT" "minus.GAT" "minus.GAT" ...
# $ query_status : chr  "minus.GAT" "minus.GAT" "minus.GAT" "minus.GAT" ...
# $ rel_freq     : num  0.0272 0.0272 0.0272 0.0272 0.0272 ...
...
#1       3   3     minus.GAT    minus.GAT 0.02720706
#2       3   3     minus.GAT    minus.GAT 0.02720706
#3       3   3     minus.GAT    minus.GAT 0.02720706
#4       3   3     minus.GAT    minus.GAT 0.02720706
#5       3   3     minus.GAT    minus.GAT 0.02720706
#6       3   3     minus.GAT    minus.GAT 0.02720706
...
summary(df.plot[!duplicated(df.plot), ])
 ...
 Min.   :   1.00   Min.   :   1.00   plus.GAT :537   plus.GAT :550  
 1st Qu.:  70.75   1st Qu.:  70.75   minus.GAT:567   minus.GAT:554  
 Median : 139.50   Median : 139.50                                  
 Mean   : 192.12   Mean   : 192.12                                  
 3rd Qu.: 212.00   3rd Qu.: 212.00                                  
 Max.   :1590.00   Max.   :1590.00                                  
    rel_freq        
 Min.   :8.025e-05  
 1st Qu.:1.605e-04  
 Median :6.421e-04  
...
library(lattice)
library(latticeExtra)
pdf('xy_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.pdf', width=15,height=5)
print(xyplot(rel_freq ~ abs.dis | anchor_status, 
         ...
         main="GATA3 peak with motif1",
         between=list(y=1.0),
         scales = list(x = list(rot = 45, relation='free', at = seq(from = 0, to = 50, by = 5))),
         par.settings = list(superpose.line = list(col=c("pink", "skyblue"), lwd=2), strip.background=list(col="grey85")),
         panel = function(x,y,...) {panel.densityplot(x, data = df.plot, 
                                                        from=0, 
                                                        to=50, 
...
png('xy_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.png')
print(xyplot(rel_freq ~ abs.dis | anchor_status, 
         #data = df.plot[!duplicated(df.plot), ],
         data = df.plot, 
         groups = query_status,
...
```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="xy plot of GATA3 peak with motif1: 3mer structure"}
#library(knitr)
knitr::include_graphics("./xy_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif1.png") 
```
...
module load bedtools 
my_motifs=("motif4" "motif2" "motif6" "motif5")

# Use a for loop to iterate over the strings
for motif in "${my_motifs[@]}"
do
  echo $motif
  awk '{print $4, $5, $6, $7, $8, $9, $10}' closest.1st.plus.GAT.to.GATA3.with.${motif}.bed | awk '{$1=$1}1' OFS="\t" | uniq | sort -k1,1 -k2,2n > closest.1st.plus.GAT.to.GATA3.with.${motif}.uniq.sorted.bed
  bedtools subtract -a hg38.3.3.3plus.36_GAT.sorted.bed -b closest.1st.plus.GAT.to.GATA3.with.${motif}.uniq.sorted.bed -f 1.00 -s > hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_${motif}.bed
  
  wc -l hg38.3.3.3plus.36_GAT.sorted.bed 
  wc -l closest.1st.plus.GAT.to.GATA3.with.${motif}.uniq.sorted.bed 
  wc -l hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_${motif}.bed 
  head -5 hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_${motif}.bed

  awk '{print $4, $5, $6, $7, $8, $9, $10}' closest.1st.minus.GAT.to.GATA3.with.${motif}.bed | awk '{$1=$1}1' OFS="\t" | uniq | sort -k1,1 -k2,2n > closest.1st.minus.GAT.to.GATA3.with.${motif}.uniq.sorted.bed
  bedtools subtract -a hg38.3.3.3minus.14_GAT.sorted.bed -b closest.1st.minus.GAT.to.GATA3.with.${motif}.uniq.sorted.bed -f 1.00 -s > hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_GATA3_with_${motif}.bed

  wc -l hg38.3.3.3minus.14_GAT.sorted.bed 
  wc -l closest.1st.minus.GAT.to.GATA3.with.motif1.uniq.sorted.bed 
  wc -l hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_GATA3_with_motif1.bed 
  head -5 hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_GATA3_with_motif1.bed

done 2>&1 | tee -a 240129_bedtools_subtract.txt
```

```{r engine='bash',  class.source = "revision", eval=F, echo=TRUE}
#cat 240129_bedtools_subtract.txt 
motif4
39090736 hg38.3.3.3plus.36_GAT.sorted.bed
6487 closest.1st.plus.GAT.to.GATA3.with.motif4.uniq.sorted.bed
39084249 hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_motif4.bed
#39090736-6487
#[1] 39084249
...
chr1	11569	11572	36	36	+	GAT
chr1	11651	11654	36	36	+	GAT
38717592 hg38.3.3.3minus.14_GAT.sorted.bed
12460 closest.1st.minus.GAT.to.GATA3.with.motif1.uniq.sorted.bed
38705132 hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_GATA3_with_motif1.bed
#38717592-12460
#[1] 38705132
...
motif2
39090736 hg38.3.3.3plus.36_GAT.sorted.bed
11464 closest.1st.plus.GAT.to.GATA3.with.motif2.uniq.sorted.bed
39079272 hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_motif2.bed
chr1	11159	11162	36	36	+	GAT
chr1	11462	11465	36	36	+	GAT
chr1	11521	11524	36	36	+	GAT
chr1	11569	11572	36	36	+	GAT
...
motif6
39090736 hg38.3.3.3plus.36_GAT.sorted.bed
5360 closest.1st.plus.GAT.to.GATA3.with.motif6.uniq.sorted.bed
39085376 hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_motif6.bed
chr1	11159	11162	36	36	+	GAT
chr1	11462	11465	36	36	+	GAT
chr1	11521	11524	36	36	+	GAT
chr1	11569	11572	36	36	+	GAT
...
motif5
39090736 hg38.3.3.3plus.36_GAT.sorted.bed
4166 closest.1st.plus.GAT.to.GATA3.with.motif5.uniq.sorted.bed
39086570 hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_motif5.bed
chr1	11159	11162	36	36	+	GAT
chr1	11462	11465	36	36	+	GAT
chr1	11521	11524	36	36	+	GAT
chr1	11569	11572	36	36	+	GAT
...
my_motifs = c("motif4","motif2", "motif6", "motif5")
for (motif in my_motifs) {
  print(motif)
  # GAT coordinates without overlapped closest GAT -- relative to G for plus, relative to C for minus
  plus.GAT.file=fiveprime.bed(read.table(file = paste0("hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_GATA3_with_", motif, ".bed"), sep="\t", header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
  minus.GAT.file=fiveprime.bed(read.table(file =paste0("hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_GATA3_with_", motif, ".bed"), sep="\t", header=FALSE), upstreamWindow = -1, downstreamWindow = 1)
  ...
  # anchor position: closest +/- GAT relative to G
  closest_plus_GAT_to_GATA3_peak_summits_=fiveprime.bed(read.table(paste0("closest.1st.plus.GAT.to.GATA3.with.", motif, ".uniq.sorted.bed"), header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
  closest_minus_GAT_to_GATA3_peak_summits=fiveprime.bed(read.table(paste0("closest.1st.minus.GAT.to.GATA3.with.", motif, ".uniq.sorted.bed"), header=FALSE), upstreamWindow = -1, downstreamWindow = 1)
  
  nrow(closest_plus_GAT_to_GATA3_peak_summits)
  head(closest_plus_GAT_to_GATA3_peak_summits) # find coordinates at Genome Browser to make sure the anchor point is correct -- anchor at G on GAT
  
  
  nrow(closest_minus_GAT_to_GATA3_peak_summits)
  head(closest_minus_GAT_to_GATA3_peak_summits) # find coordinates at Genome Browser to make sure the anchor point is correct -- anchor at C on ATC
  
  
  
  # 2nd closest +/-GAT to closest plus GAT
  
  closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.with.motif=bedTools.closest(bed1 = closest_plus_GAT_to_GATA3_peak_summits[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')
  closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.with.motif=bedTools.closest(bed1 = closest_minus_GAT_to_GATA3_peak_summits[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')
  
  
  write.table(closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.with.motif,file= paste0('closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.', motif, '.bed'), quote=F,sep="\t",col.names=F,row.names=F)
  write.table(closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.with.motif,file= paste0('closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.', motif, '.bed'), quote=F,sep="\t",col.names=F,row.names=F)
  
  # 2nd closest +/- GAT to closest minus GAT
  closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.with.motif=bedTools.closest(bed1 = closest_plus_GAT_to_GATA3_peak_summits[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')
  closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.with.motif=bedTools.closest(bed1 = closest_minus_GAT_to_GATA3_peak_summits[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')
  
  
  write.table(closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.with.motif,file= paste0('closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.', motif, '.bed'), quote=F,sep="\t",col.names=F,row.names=F)
  write.table(closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.with.motif,file= paste0('closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.', motif, '.bed'), quote=F,sep="\t",col.names=F,row.names=F)
  
}

save.image('240129_GATA3_ChIP_2nd_clsestbed2.Rdata') 
```
...
df.plot = data.frame(matrix(nrow = 0, ncol = 3))     
colnames(df.plot) = c("dis", "anchor_status", "query_status")
for (closest_2nd_dis in Sys.glob(file.path("./closest.2nd*GAT.to.1st*GAT.GATA3.motif4.bed"))) {
    print(closest_2nd_dis)
    anchor_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.*.to.1st.')[[1]][2]), ".GATA3.motif4.bed")[[1]][1]
    print(anchor_status)
    query_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.')[[1]][2]), ".to.1st.*.GATA3.motif4.bed")[[1]][1]
    print(query_status)
    temp = cbind(read.table(closest_2nd_dis, header=F, comment.char='')[,11], anchor_status, query_status)
    df.plot = rbind(df.plot,temp)
}
...
head(df.plot)
#dis anchor_status query_status
#1   4     minus.GAT    minus.GAT
#2  12     minus.GAT    minus.GAT
#3  39     minus.GAT    minus.GAT
#4   4     minus.GAT    minus.GAT
#5   7     minus.GAT    minus.GAT
#6 110     minus.GAT    minus.GAT
...
library(latticeExtra)
  # PDF plot
  pdf('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif4.pdf', width=15,height=5)
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
  ...
  dev.off()
   ...
   png('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif4.png')
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
...
df.plot = data.frame(matrix(nrow = 0, ncol = 3))     
colnames(df.plot) = c("dis", "anchor_status", "query_status")
for (closest_2nd_dis in Sys.glob(file.path("./closest.2nd*GAT.to.1st*GAT.GATA3.motif2.bed"))) {
    print(closest_2nd_dis)
    anchor_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.*.to.1st.')[[1]][2]), ".GATA3.motif2.bed")[[1]][1]
    print(anchor_status)
    query_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.')[[1]][2]), ".to.1st.*.GATA3.motif2.bed")[[1]][1]
    print(query_status)
    temp = cbind(read.table(closest_2nd_dis, header=F, comment.char='')[,11], anchor_status, query_status)
    df.plot = rbind(df.plot,temp)
}
...
head(df.plot)
#  dis anchor_status query_status
#1 117     minus.GAT    minus.GAT
#2   6     minus.GAT    minus.GAT
#3   6     minus.GAT    minus.GAT
#4  17     minus.GAT    minus.GAT
#5  14     minus.GAT    minus.GAT
#6  14     minus.GAT    minus.GAT
...
library(latticeExtra)
  # PDF plot
  pdf('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif2.pdf', width=15,height=5)
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
            ...
            panel = function(x, ...) {
                                      panel.densityplot(x, ...)
                                      panel.ablineq(v = 8, col = "red", lty = 2, adj=c(0,1), label="8")
                                      panel.ablineq(v = 9, col = "red", lty = 2, adj=c(0,1), label="9")} # change to actual y relative to each distance x values; xyplot
            
))
  dev.off()
  
   png('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif2.png')
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
            ...
            panel = function(x, ...) {
                                      panel.densityplot(x, ...)
                                      panel.ablineq(v = 8, col = "red", lty = 2, adj=c(0,1), label="8")
                                      panel.ablineq(v = 9, col = "red", lty = 2, adj=c(0,1), label="9")}
...
df.plot = data.frame(matrix(nrow = 0, ncol = 3))     
colnames(df.plot) = c("dis", "anchor_status", "query_status")
for (closest_2nd_dis in Sys.glob(file.path("./closest.2nd*GAT.to.1st*GAT.GATA3.motif6.bed"))) {
    print(closest_2nd_dis)
    anchor_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.*.to.1st.')[[1]][2]), ".GATA3.motif6.bed")[[1]][1]
    print(anchor_status)
    query_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.')[[1]][2]), ".to.1st.*.GATA3.motif6.bed")[[1]][1]
    print(query_status)
    temp = cbind(read.table(closest_2nd_dis, header=F, comment.char='')[,11], anchor_status, query_status)
    df.plot = rbind(df.plot,temp)
}
...
head(df.plot)
# dis anchor_status query_status
#1   9     minus.GAT    minus.GAT
#2   9     minus.GAT    minus.GAT
#3 166     minus.GAT    minus.GAT
#4   9     minus.GAT    minus.GAT
#5  15     minus.GAT    minus.GAT
#6  10     minus.GAT    minus.GAT
...
library(latticeExtra)
  # PDF plot
  pdf('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif6.pdf', width=15,height=5)
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
  ...
  dev.off()
   ...
   png('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif6.png')
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
...
df.plot = data.frame(matrix(nrow = 0, ncol = 3))     
colnames(df.plot) = c("dis", "anchor_status", "query_status")
for (closest_2nd_dis in Sys.glob(file.path("./closest.2nd*GAT.to.1st*GAT.GATA3.motif5.bed"))) {
    print(closest_2nd_dis)
    anchor_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.*.to.1st.')[[1]][2]), ".GATA3.motif5.bed")[[1]][1]
    print(anchor_status)
    query_status =strsplit((strsplit(strsplit(closest_2nd_dis, "/")[[1]][length(strsplit(closest_2nd_dis, "/")[[1]])], 'closest.2nd.')[[1]][2]), ".to.1st.*.GATA3.motif5.bed")[[1]][1]
    print(query_status)
    temp = cbind(read.table(closest_2nd_dis, header=F, comment.char='')[,11], anchor_status, query_status)
    df.plot = rbind(df.plot,temp)
}
...
head(df.plot)
# dis anchor_status query_status
#1  45     minus.GAT    minus.GAT
#2   7     minus.GAT    minus.GAT
#3  47     minus.GAT    minus.GAT
#4   5     minus.GAT    minus.GAT
#5  41     minus.GAT    minus.GAT
#6  10     minus.GAT    minus.GAT
...
library(latticeExtra)
  # PDF plot
  pdf('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif5.pdf', width=15,height=5)
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
            ...
            panel = function(x, ...) {
                                      panel.densityplot(x, ...)
                                      panel.ablineq(v = 2, col = "red", lty = 2, adj=c(0,1), label="2")
                                      panel.ablineq(v = 5, col = "red", lty = 2, adj=c(0,1), label="5")
                                      panel.ablineq(v = 9, col = "red", lty = 2, adj=c(0,1), label="9")}
            
))
  dev.off()
  
   png('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif5.png')
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
...
**alternative PDFs with data points** \
```{r engine='R',  class.source = "revision", eval=F, echo=TRUE}
my_motifs = c("motif4","motif2", "motif6", "motif5")
for (motif in my_motifs) {
  print(motif)
...
library(lattice)
library(latticeExtra)
pdf(paste0('PDF2_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_', motif, '.pdf'), width=15,height=5)
print(xyplot(density ~ abs.dis | anchor_status, 
         data = df.plot,
         groups = query_status,
         auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
         ...
         scales = list(x = list(rot = 45, at = seq(from = 0, to = 50, by = 5))),
         par.settings = list(superpose.line = list(col=c("pink", "skyblue"), lwd=2), strip.background=list(col="grey85")),
         panel = function(x, y, ...) {panel.densityplot(x, 
                                                        data = df.plot, 
                                                        from=0, 
                                                        to=50,
                                                        bw="nrd0",
...
```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="xy plot of GATA3 peak with motif4: 3mer structure"}
#library(knitr)
knitr::include_graphics("./xy_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif4.png") 
```

```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="xy plot of GATA3 peak with motif2: 3mer structure"}
#library(knitr)
knitr::include_graphics("./xy_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif2.png") 
```

```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="xy plot of GATA3 peak with motif6: 3mer structure"}
#library(knitr)
knitr::include_graphics("./xy_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif6.png") 
```

```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="xy plot of GATA3 peak with motif5: 3mer structure"}
#library(knitr)
knitr::include_graphics("./xy_closest_2nd_GAT_to_closest_1st_GAT_to_GATA3_pos_motif5.png") 
```
...
#dir=/labs/Guertin/siyu/Sathyan_GATA3_ChIP_pool1_pool2/overrep_3mer/hg38_full_kmer3_rs1000/seqdump/
#sort -k1,1 -k2,2n ${dir}hg38.3.3.3plus.36_GAT.bed > hg38.3.3.3plus.36_GAT.sorted.bed
awk '{print $4, $5, $6, $7, $8, $9, $10}' closest.1st.plus.GAT.to.indep.DHS.control.consensus.bed | awk '{$1=$1}1' OFS="\t" | uniq | sort -k1,1 -k2,2n > closest.1st.plus.GAT.to.indep.DHS.control.consensus.uniq.sorted.bed
bedtools subtract -a hg38.3.3.3plus.36_GAT.sorted.bed -b closest.1st.plus.GAT.to.indep.DHS.control.consensus.uniq.sorted.bed -f 1.00 -s > hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_indep_DHS_control.bed

wc -l hg38.3.3.3plus.36_GAT.sorted.bed #39090736
wc -l closest.1st.plus.GAT.to.indep.DHS.control.consensus.uniq.sorted.bed #57261
wc -l hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_indep_DHS_control.bed #39033475
head -5 hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_indep_DHS_control.bed
#chr1	11159	11162	36	36	+	GAT
#chr1	11462	11465	36	36	+	GAT
#chr1	11521	11524	36	36	+	GAT
#chr1	11569	11572	36	36	+	GAT
...
#sort -k1,1 -k2,2n ${dir}hg38.3.3.3minus.14_GAT.bed > hg38.3.3.3minus.14_GAT.sorted.bed
awk '{print $4, $5, $6, $7, $8, $9, $10}' closest.1st.minus.GAT.to.indep.DHS.control.consensus.bed | awk '{$1=$1}1' OFS="\t" | uniq | sort -k1,1 -k2,2n > closest.1st.minus.GAT.to.indep.DHS.control.consensus.uniq.sorted.bed
bedtools subtract -a hg38.3.3.3minus.14_GAT.sorted.bed -b closest.1st.minus.GAT.to.indep.DHS.control.consensus.uniq.sorted.bed -f 1.00 -s > hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_indep_DHS_control.bed

wc -l hg38.3.3.3minus.14_GAT.sorted.bed #38717592
wc -l closest.1st.minus.GAT.to.indep.DHS.control.consensus.uniq.sorted.bed #57251 
wc -l hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_indep_DHS_control.bed #38660341

head -5 hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_indep_DHS_control.bed 
#chr1	10545	10548	14	14	-	GAT
#chr1	11145	11148	14	14	-	GAT
#chr1	11160	11163	14	14	-	GAT
#chr1	11576	11579	14	14	-	GAT
...
library(bigWig)
# GAT coordinates without overlapped closest GAT -- relative to G for plus, relative to C for minus
plus.GAT.file=fiveprime.bed(read.table(file = "hg38.3.3.3plus.36_GAT_without_1st_plus_GAT_to_indep_DHS_control.bed", sep="\t", header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
minus.GAT.file=fiveprime.bed(read.table(file = "hg38.3.3.3minus.14_GAT_without_1st_minus_GAT_to_indep_DHS_control.bed", sep="\t", header=FALSE), upstreamWindow = -1, downstreamWindow = 1)
...
# anchor position: closest +/- GAT relative to G
closest_plus_GAT_to_indep_DHS=fiveprime.bed(read.table("closest.1st.plus.GAT.to.indep.DHS.control.consensus.uniq.sorted.bed", header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
closest_minus_GAT_to_indep_DHS=fiveprime.bed(read.table("closest.1st.minus.GAT.to.indep.DHS.control.consensus.uniq.sorted.bed", header=FALSE), upstreamWindow = -1, downstreamWindow = 1)

nrow(closest_plus_GAT_to_indep_DHS)

head(closest_plus_GAT_to_indep_DHS) # find coordinates at Genome Browser to make sure the anchor point is correct -- anchor at G on GAT


nrow(closest_minus_GAT_to_indep_DHS)

head(closest_minus_GAT_to_indep_DHS) # find coordinates at Genome Browser to make sure the anchor point is correct -- anchor at C on ATC


# 2nd closest +/-GAT to closest plus GAT

closest.2nd.plus.GAT.to.1st.plus.GAT.indep.DHS=bedTools.closest(bed1 = closest_plus_GAT_to_indep_DHS[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')
closest.2nd.minus.GAT.to.1st.plus.GAT.indep.DHS=bedTools.closest(bed1 = closest_minus_GAT_to_indep_DHS[,1:3], bed2 = plus.GAT.file, opt.string = '-d -t first')



write.table(closest.2nd.plus.GAT.to.1st.plus.GAT.indep.DHS,file= 'closest.2nd.plus.GAT.to.1st.plus.GAT.indep.DHS.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.2nd.minus.GAT.to.1st.plus.GAT.indep.DHS,file= 'closest.2nd.minus.GAT.to.1st.plus.GAT.indep.DHS.bed', quote=F,sep="\t",col.names=F,row.names=F)

# 2nd closest +/- GAT to closest minus GAT
closest.2nd.plus.GAT.to.1st.minus.GAT.indep.DHS=bedTools.closest(bed1 = closest_plus_GAT_to_indep_DHS[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')
closest.2nd.minus.GAT.to.1st.minus.GAT.indep.DHS=bedTools.closest(bed1 = closest_minus_GAT_to_indep_DHS[,1:3], bed2 = minus.GAT.file, opt.string = '-d -t first')



write.table(closest.2nd.plus.GAT.to.1st.minus.GAT.indep.DHS,file= 'closest.2nd.plus.GAT.to.1st.minus.GAT.indep.DHS.bed', quote=F,sep="\t",col.names=F,row.names=F)
write.table(closest.2nd.minus.GAT.to.1st.minus.GAT.indep.DHS,file= 'closest.2nd.minus.GAT.to.1st.minus.GAT.indep.DHS.bed', quote=F,sep="\t",col.names=F,row.names=F)


save.image('240129_GATA3_ChIP_2nd_clsestbed3.Rdata') 
```
...
head(df.plot)
#dis anchor_status query_status
#1  17     minus.GAT    minus.GAT
#2  23     minus.GAT    minus.GAT
#3  62     minus.GAT    minus.GAT
#4  17     minus.GAT    minus.GAT
#5  83     minus.GAT    minus.GAT
#6  25     minus.GAT    minus.GAT
nrow(df.plot)
#[1] 229024
...
library(latticeExtra)
  # PDF plot
  pdf('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_indep_DHS.pdf', width=15,height=5)
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
            ...
            lty = c(1),
            lwd=2,
            par.settings = list(superpose.line = list(col=c("pink", "skyblue"), lwd=3), strip.background=list(col="grey85"))
           
            
))
  dev.off()
  
   png('PDF_closest_2nd_GAT_to_closest_1st_GAT_to_indep_DHS.png')
  print(densityplot( ~ abs(dis) | anchor_status,
            groups = query_status,
            data = df.plot,
            auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
...
```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="independent DHS regions: 3mer structure"}
library(knitr)
knitr::include_graphics("./PDF_closest_2nd_GAT_to_closest_1st_GAT_to_indep_DHS.png") 
```

**Add actual data point to the density plot:** \
```{r engine='R',  class.source = "revision", eval=F, echo=TRUE}
df.plot = data.frame(matrix(nrow = 0, ncol = 5))     
colnames(df.plot) = c("dis","anchor_status", "query_status","abs.dis", "density")
for (closest_2nd_dis in Sys.glob(file.path("./closest.2nd*GAT.to.1st*GAT.indep.DHS.bed"))) {
...
head(df.plot)
#dis anchor_status query_status abs.dis     density
#1  17     minus.GAT    minus.GAT      17 0.009672190
#2  23     minus.GAT    minus.GAT      23 0.009327138
#3  62     minus.GAT    minus.GAT      62          NA
#4  17     minus.GAT    minus.GAT      17 0.009672190
#5  83     minus.GAT    minus.GAT      83          NA
#6  25     minus.GAT    minus.GAT      25 0.009207771
nrow(df.plot)
#[1] 229024
...
library(lattice)
library(latticeExtra)
pdf('PDF2_closest_2nd_GAT_to_closest_1st_GAT_to_indep_DHS.pdf', width=15,height=5)
print(xyplot(density ~ abs.dis | anchor_status, 
         data = df.plot,
         groups = query_status,
         auto.key = list(space = "right", lines=TRUE, points=FALSE, cex = 1),
...
```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="xy plot of Independent DHS Regions: 3mer structure"}
#library(knitr)
knitr::include_graphics("./xy_closest_2nd_GAT_to_closest_1st_GAT_to_indep_DHS.png") 
```


### UCSC Genome Browser
#### Visualize the 2nd closest 3mer-GAT distribution relative to the defined "closest GAT" on Genome Browser

files to upload: \
-peak summit BED file. \
...
**GAT coordinates files: convert to bigWig** \
sort the .bed file: \
```{r engine='bash', eval=F, echo=TRUE}
#GAT
dir=/labs/Guertin/siyu/Sathyan_GATA3_ChIP_pool1_pool2/overrep_3mer/hg38_full_kmer3_rs1000/seqdump/
...
sort -k1,1 -k2,2n ${plus_file} > hg38.3.3.3.1000.plus.36_GAT_sorted.bed
sort -k1,1 -k2,2n ${minus_file} > hg38.3.3.3.1000.minus.14_GAT_sorted.bed
wc -l hg38.3.3.3.1000.plus.36_GAT_sorted.bed
wc -l hg38.3.3.3.1000.minus.14_GAT_sorted.bed
```
...
bedtools genomecov -bg -i hg38.3.3.3.1000.plus.36_GAT_sorted.bed -g ${sizes} > hg38.3.3.3.1000.plus.36_GAT.bedGraph 
bedtools genomecov -bg -i hg38.3.3.3.1000.minus.14_GAT_sorted.bed -g ${sizes} > hg38.3.3.3.1000.minus.14_GAT.bedGraph 
wc -l hg38.3.3.3.1000.plus.36_GAT.bedGraph 
wc -l hg38.3.3.3.1000.minus.14_GAT.bedGraph 
```

The final step is to convert .bedGraph to .bigWig with UCSC `bedGraphToBigWig` tool.
```{r engine='bash', eval=F, echo=TRUE}
#UCSC bedGraphToBigWig tool
bedGraphToBigWig hg38.3.3.3.1000.plus.36_GAT.bedGraph ${sizes} hg38.3.3.3.1000.plus.36_GAT.bigWig
bedGraphToBigWig hg38.3.3.3.1000.minus.14_GAT.bedGraph ${sizes} hg38.3.3.3.1000.minus.14_GAT.bigWig
```

```{r engine='bash', eval=TRUE, echo=TRUE}
#cat UCSCGB.sh_7661175.err
cat UCSCGB.sh_7661175.out
```
...
```{r, engine='R', eval=F, echo=TRUE}
library(bigWig)
peak.region.summit=center.bed(read.table('/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/GAT_3mer_analaysis/MAST_positive_control/GATA3_peak_161win_with_motif_4.bed', sep = "\t", header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
write.table(peak.region.summit,file= 'GATA3_peak_summit_with_motif_4.bed', quote=F,sep="\t",col.names=F,row.names=F)

head -5 GATA3_peak_summit_with_motif_4.bed 
#chr10	101454261	101454262	GATA_ChIP_peak_10760c
#chr10	101703137	101703138	GATA_ChIP_peak_10768a
#chr10	101716956	101716957	GATA_ChIP_peak_10770
#chr10	102509735	102509736	GATA_ChIP_peak_10791b
#chr10	1025947	1025948	GATA_ChIP_peak_7503

```

```{r engine='bash', eval=F, echo=TRUE}
# 1) peak summit files
cat GATA3_peak_summit_with_motif_4.bed | sort -k1,1 -k2,2n > GATA3_peak_summit_with_motif_4.sorted.bed

# 2) closest GAT to peak summit (+/-)
awk '{print $4, $5, $6, $7, $8, $9}' closest.1st.plus.GAT.to.GATA3.with.motif4.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.1st.plus.GAT.to.GATA3.with.motif4.sorted.bed

awk '{print $4, $5, $6, $7, $8, $9}' closest.1st.minus.GAT.to.GATA3.with.motif4.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.1st.minus.GAT.to.GATA3.with.motif4.sorted.bed


# 3) 2nd closest GAT to peak summit (+/-)

awk '{print $4, $5, $6, $7, $8, $9}' closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif4.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif4.sorted.bed

awk '{print $4, $5, $6, $7, $8, $9}' closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif4.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif4.sorted.bed

awk '{print $4, $5, $6, $7, $8, $9}' closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif4.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif4.sorted.bed

awk '{print $4, $5, $6, $7, $8, $9}' closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif4.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif4.sorted.bed
```

Add trackline: \
```{r engine='bash', eval=F, echo=TRUE}
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"GATA3.peak.with.motif4.summit.bed\" description=\"peak.motif4.summit_bed\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' GATA3_peak_summit_with_motif_4.sorted.bed > GATA3_peak_summit_with_motif_4.sorted.header.bed
           
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.1st.plus.GAT.motif4.bed\" description=\"plus.1st.GAT.m4\" visibility=full autoScale=on useScore=1 color=255,0,0"
            } {print $0}' closest.1st.plus.GAT.to.GATA3.with.motif4.sorted.bed > closest.1st.plus.GAT.to.GATA3.with.motif4.sorted.header.bed
           
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.1st.minus.GAT.motif4.bed\" description=\"minus.1st.GAT.m4\" visibility=full autoScale=on useScore=1 color=0,0,255"
            } {print $0}' closest.1st.minus.GAT.to.GATA3.with.motif4.sorted.bed > closest.1st.minus.GAT.to.GATA3.with.motif4.sorted.header.bed
            

awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif4.bed\" description=\"plus.2nd.GAT.to.plus.1st.m4\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif4.sorted.bed > closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif4.sorted.header.bed
            

awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif4.bed\" description=\"plus.2nd.GAT.to.minus.1st.m4\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif4.sorted.bed > closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif4.sorted.header.bed
            
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif4.bed\" description=\"minus.2nd.GAT.to.plus.1st.m4\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif4.sorted.bed > closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif4.sorted.header.bed
            
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif4.bed\" description=\"minus.2nd.GAT.to.minus.1st.m4\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif4.sorted.bed > closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif4.sorted.header.bed
...
```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="UCSC Genome browser image: 2nd closest GAT to 1st GAT relaive to GATA3 peaks with motif4"}
#library(knitr)
knitr::include_graphics("./GATA3_peak_with_motif4_2nd_closest_GAT_8bp.png") 
```
...
```{r, engine='R', eval=F, echo=TRUE}
library(bigWig)
peak.region.summit=center.bed(read.table('/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/GAT_3mer_analaysis/MAST_positive_control/GATA3_peak_161win_with_motif_1.bed', sep = "\t", header=FALSE), upstreamWindow = 0, downstreamWindow = 0)
write.table(peak.region.summit,file= 'GATA3_peak_summit_with_motif_1.bed', quote=F,sep="\t",col.names=F,row.names=F)

head -5 GATA3_peak_summit_with_motif_1.bed 
#chr10	100072672	100072673	GATA_ChIP_peak_10714
#chr10	100522507	100522508	GATA_ChIP_peak_10733
#chr10	101380202	101380203	GATA_ChIP_peak_10758b
#chr10	101453916	101453917	GATA_ChIP_peak_10760b
#chr10	101892965	101892966	GATA_ChIP_peak_10774

```

```{r engine='bash', eval=F, echo=TRUE}
# 1) peak summit files
cat GATA3_peak_summit_with_motif_1.bed | sort -k1,1 -k2,2n > GATA3_peak_summit_with_motif_1.sorted.bed

# 2) closest GAT to peak summit (+/-)
awk '{print $4, $5, $6, $7, $8, $9}' closest.1st.plus.GAT.to.GATA3.with.motif1.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.1st.plus.GAT.to.GATA3.with.motif1.sorted.bed

awk '{print $4, $5, $6, $7, $8, $9}' closest.1st.minus.GAT.to.GATA3.with.motif1.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.1st.minus.GAT.to.GATA3.with.motif1.sorted.bed


# 3) 2nd closest GAT to peak summit (+/-)

awk '{print $4, $5, $6, $7, $8, $9}' closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif1.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif1.sorted.bed

awk '{print $4, $5, $6, $7, $8, $9}' closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif1.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif1.sorted.bed

awk '{print $4, $5, $6, $7, $8, $9}' closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif1.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif1.sorted.bed

awk '{print $4, $5, $6, $7, $8, $9}' closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif1.bed | awk '{$1=$1}1' OFS="\t" | sort -k1,1 -k2,2n > closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif1.sorted.bed
```

Add trackline: \
```{r engine='bash', eval=F, echo=TRUE}
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"GATA3.peak.with.motif1.summit.bed\" description=\"peak.motif1.summit_bed\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' GATA3_peak_summit_with_motif_1.sorted.bed > GATA3_peak_summit_with_motif_1.sorted.header.bed
           
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.1st.plus.GAT.motif1.bed\" description=\"plus.1st.GAT.m1\" visibility=full autoScale=on useScore=1 color=255,0,0"
            } {print $0}' closest.1st.plus.GAT.to.GATA3.with.motif1.sorted.bed > closest.1st.plus.GAT.to.GATA3.with.motif1.sorted.header.bed
           
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.1st.minus.GAT.motif1.bed\" description=\"minus.1st.GAT.m1\" visibility=full autoScale=on useScore=1 color=0,0,255"
            } {print $0}' closest.1st.minus.GAT.to.GATA3.with.motif1.sorted.bed > closest.1st.minus.GAT.to.GATA3.with.motif1.sorted.header.bed
            

awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif1.bed\" description=\"plus.2nd.GAT.to.plus.1st.m1\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif1.sorted.bed > closest.2nd.plus.GAT.to.1st.plus.GAT.GATA3.motif1.sorted.header.bed
            

awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif1.bed\" description=\"plus.2nd.GAT.to.minus.1st.m1\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif1.sorted.bed > closest.2nd.plus.GAT.to.1st.minus.GAT.GATA3.motif1.sorted.header.bed
            
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif1.bed\" description=\"minus.2nd.GAT.to.plus.1st.m1\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif1.sorted.bed > closest.2nd.minus.GAT.to.1st.plus.GAT.GATA3.motif1.sorted.header.bed
            
awk 'BEGIN {print "browser position chr10:16,000-17,000" 
            print "track type=bed name=\"closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif1.bed\" description=\"minus.2nd.GAT.to.minus.1st.m1\" visibility=full autoScale=on useScore=1 color=0,0,0"
            } {print $0}' closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif1.sorted.bed > closest.2nd.minus.GAT.to.1st.minus.GAT.GATA3.motif1.sorted.header.bed
...
```{r  fig.align = "center", out.width="80%", out.height="80%", echo=F, fig.align = "center", fig.cap="UCSC Genome browser image: 2nd closest GAT to 1st GAT relaive to GATA3 peaks with motif1"}
#library(knitr)
knitr::include_graphics("./GATA3_peak_with_motif1_2nd_closest_GAT_8bp.png") 
```


## other 3mers--prioritized list of 3mers
...
## GATA3 structure and protein-DNA interaction

**4HC7: Complex1** \
Complex1 is showing N terminal ZnFn1 binding to DNA at major groove through hydrogen bond with a R-C, N-A, R-T, R-C; \
and the C terminal ZnFn2 binding to DNA at major groove through hydrogen bond with a R330K-G, TR330-rvA(T), N-rvT(A), RR-rvA(T), RN-A(HK-rvA(T)); \

```{r  out.width = "80%", out.height="80%", echo=F, fig.align = "center", fig.cap="Complex1 GATA3 ZnFn1 binds to DNA cCATC, ZnFn2 binds to DNA GATAA"}
#library(knitr)
knitr::include_graphics("./4HCA_complex1_ZnFn12.png") 
```


```{r  out.width = "80%", out.height="80%", echo=F, fig.align = "center", fig.cap="Complex1 GATA3 potein-DNA interaction"}
#library(knitr)
knitr::include_graphics("./4HCA_complex1_GATA3_DNA_interaction.png") 
```


**4HC7: Complex2** \
Complex2 bottom part is showing N terminal ZnFn1 binding to DNA at major groove through hydrogen bond with a R-A, NA-A, R-T, R-rvC(A); \
and the C terminal ZnFn2 binding to DNA (GATAA) at major groove through hydrogen bond with a R330K-G, R330-rvA(T), N-rvT(A), R353-rvA(T), H-rvA(T). \

```{r  out.width = "80%", out.height="80%", echo=F, fig.align = "center", fig.cap="Complex2 GATA3 ZnFn2 binds to DNA AATC"}
#library(knitr)
knitr::include_graphics("./4HC7_complex2_ZnFn1_dAATC.png") 
```

```{r  out.width = "80%", out.height="80%", echo=F, fig.align = "center", fig.cap="Complex2 GATA3 ZnFn2 binds to DNA GATAA"}
#library(knitr)
knitr::include_graphics("./4HC7_complex2_ZnFn2_dGATAA.png") 
```

```{r  out.width = "80%", out.height="80%", echo=F, fig.align = "center", fig.cap="Complex2 GATA3 potein-DNA interaction"}
#library(knitr)
knitr::include_graphics("./4HC7_complex2_GATA3_DNA_interaction.png") 
```


**4HC9: Complex3** \
Complex 3 is showing N terminal ZnFn1 binding to DNA at major groove through hydrogen bond with a N-dA, R-dT, L-dC interaction. \
```{r  out.width = "80%", out.height="80%", echo=F, fig.align = "center", fig.cap="Complex3 GATA3 ZnFn1 binds to DNA ATC"}
#library(knitr)
knitr::include_graphics("./4HC9_complex3_ZnFn1_dATC.png") 
```

**Homology of two GATA3 zinc fingers:** \

N finger: The conserved binding element is "GAT/ATC". G or C is mostly bind by Arginine(R) in the beta sheets, A or T has been seen bind by Asparagine(N) and Arginine(R) in alpha helix. \
C finger: The conserved binding element is "GATAA/TTATC". Conserved amino acids binding to this DNA sequences are Arginine(R), Asparagine(N), lysine(K) and Histidine (H). \

## Software to search for variable spacing motifs. 
**RSAT (Regulatory Sequence Analysis Tools)- dyad analysis:** \
RSAT provides a collection of tools for the analysis of regulatory sequences. It includes tools like dyad-analysis, which allows for the detection of transcription factor binding sites with spaced pairs of k-mers. \

http://rsat.sb-roscoff.fr/dyad-analysis_form.cgi \
```{r engine='bash', eval=F, echo=TRUE}
module load bedtools
genome=/home/FCAM/ssun/Genome/hg38.fa
fastaFromBed -fi $genome -bed with_motifs_1_101bp_mast.bed -fo with_motifs_1_101bp_mast.fasta
```

### GATA3 activaton domain

### install

refer to: https://rsa-tools.github.io/installing-RSAT/conda-install-rsat/bioconda-rsat-core.html \
```{r engine='bash', eval=F, echo=TRUE}
cd /home/FCAM/ssun/packages/
source ~/miniconda3/bin/activate
conda create -n rsat
conda env list | grep rsat
#rsat                     /home/FCAM/ssun/miniconda3/envs/rsat
conda activate rsat
conda list # see what package listed in the env
conda install -c bioconda rsat-core

#testing
which rsat
#~/miniconda3/envs/rsat/bin/rsat     
...
**Generating the expected frequency table** \

The RSAT-dyad analysis provide two ways to control for background. One is using the `-bg` option: Either using the predefined background frequencies of the selected organism (`-org`)/taxon(`-taxon`), or estimating expected dyad frequencies from input sequences (`-bg monads` (default)/`-bg input`). \

The other one allows users to upload their own expected frequency file with `-expfreq`. \
On the websites, they have a detailed instruction of "How to generate an expected frequency file?". I will follow their rules to generate the expected frequency table use the DHS regions. \

```{r engine='bash', eval=F, echo=TRUE}
#A sample background file on the website:
head -50 3nt_gene_Saccharomyces_cerevisiae-ovlp-1str.freq 

; oligo-analysis  -v 1 -1str -i /bio/rsa-tools/data/genomes/Saccharomyces_cerevisiae/genome/Saccharomyces_cerevisiae_gene_segments.wc -format wc -1str -ovlp -l 3 -type dna -return freq,occ -o /bio/rsa-tools/data/genomes/Saccharomyces_cerevisiae/oligo-frequencies/3nt_gene_Saccharomyces_cerevisiae-ovlp-1str.freq
; Citation: van Helden et al. (1998). J Mol Biol 281(5), 827-42. 
; Oligomer length              	3
; Input file                   	/bio/rsa-tools/data/genomes/Saccharomyces_cerevisiae/genome/Saccharomyces_cerevisiae_gene_segments.wc
; Input format                 	wc
; Output file                  	/bio/rsa-tools/data/genomes/Saccharomyces_cerevisiae/oligo-frequencies/3nt_gene_Saccharomyces_cerevisiae-ovlp-1str.freq
; Count overlapping matches
; Counted on a single strand
; Method                       	
; Sequence type                	DNA
; Nb of sequences              	6080
; Sum of sequence lengths      	8809751
; discarded residues           	0	 (other letters than ACGT)
; discarded occurrences        	0	 (contain discarded residues)
; nb possible positions        	8797591
; total oligo occurrences      	8797591
; alphabet size                	4
...
;	5	test           	over- or under-representation test
;seq	id	observed_freq	occ	test
aaa	aaa	0.0350766476868	308590	
aac	aac	0.0184259531956	162104	
aag	aag	0.0218185864744	191951	
aat	aat	0.0282970645032	248946	
aca	aca	0.0168762107718	148470	
acc	acc	0.0126323217344	111134	
acg	acg	0.0088157087548	77557	
act	act	0.0149614820694	131625	
aga	aga	0.0207571595452	182613	
agc	agc	0.0118527901559	104276	
agg	agg	0.0118003894475	103815	
agt	agt	0.0150120640980	132070	
ata	ata	0.0206869130424	181995	
atc	atc	0.0186547658331	164117	
atg	atg	0.0186416940728	164002	
att	att	0.0282246583184	248309	
caa	caa	0.0245105734058	215634	
cac	cac	0.0108630874065	95569	
cag	cag	0.0135514369786	119220	
cat	cat	0.0187047795243	164557
```


`create-background-model`: Create a background model by computing oligonucleotide frequencies in a user-specified sequence file (he "background" file). \
```{r engine='bash', eval=F, echo=TRUE}
#rsat convert-background-model -h
#rsat create-background-model -h
...
fastaFromBed -fi $genome -bed ${dir}MCF7DHS_consensus_noGATA_without_motifs_123456_78.bed -fo MCF7DHS_consensus_noGATA_without_motifs_123456_78.fasta

rsat create-background-model -i MCF7DHS_consensus_noGATA_without_motifs_123456_78.fasta -o ENCODE.MCF7.DHS.background.txt -out_format oligos -markov 2
#An oligo file containing k-mers frequencies corresponds to a Markov model of order m=k-1.


rsat oligo-analysis -i MCF7DHS_consensus_noGATA_without_motifs_123456_78.fasta -seqtype dna -l 3 -1str -return freq -lth occ_sig none -v 1 -format fasta -sort -o ENCODE.MCF7.DHS.background2.txt
#-noov	do not allow overlapping matches of the same word
#-ovlp Count overlapping occurrences of self-overlpping words.

#-1str Strand-sensitive background model
#	-1str inactivates the summation of occurrences on both strands. Count on single strand
```

```{r engine='bash', eval=F, echo=TRUE}
head -30 ENCODE.MCF7.DHS.background2.txt 

; oligo-analysis  -i MCF7DHS_consensus_noGATA_without_motifs_123456_78.fasta -seqtype dna -l 3 -1str -return freq -lth occ_sig none -v 1 -format fasta -sort -o ENCODE.MCF7.DHS.background2.txt
; Citation: van Helden et al. (1998). J Mol Biol 281(5), 827-42. 
; Program version              	1.169
; Slow counting mode           
; Oligomer length              	3
; Input file                   	MCF7DHS_consensus_noGATA_without_motifs_123456_78.fasta
; Input format                 	fasta
; Output file                  	ENCODE.MCF7.DHS.background2.txt
; Count overlapping matches
; Counted on a single strand
; Background model             	Bernoulli
; Background estimation method 	
; Sequence type                	DNA
; Nb of sequences              	57906
; Sum of sequence lengths      	7677291
; discarded residues           		 (other letters than ACGT)
; discarded occurrences        		 (contain discarded residues)
; nb possible positions        	7561762
; total oligo occurrences      	7561762
; alphabet size                	4
; nb possible oligomers        	64
; oligomers tested for significance	0
;
...
#seq	id	obs_freq
ctg	ctg	0.0290296097656
cag	cag	0.0289211694311

```

The 3mer oligo frequencies stored in ENCODE.MCF7.DHS.background2.txt generated by `rsat oligo-analysis` is the same as the 3mer oligo frequencies stored in file generated by `rsat create-background-model`. \

```{r engine='bash', eval=F, echo=TRUE}
awk '{print $1, $3}' ENCODE.MCF7.DHS.background.txt | awk '{$1=$1}1' OFS="\t" > ENCODE.MCF7.DHS.background3.txt
```

**Estimation of expected dyad frequencies** \
```{r engine='bash', eval=F, echo=TRUE}
rsat dyad-analysis  -v 1 -i MCF7DHS_consensus_noGATA_without_motifs_123456_78.fasta -format fasta -type any -seqtype dna -1str -ovlp -sp 0-20 -l 3 -return freq,occ -o ENCODE.MCF7.DHS.background4.txt
#-ovlp Count overlapping occurrences of self-overlpping words.
```


**Perform RSAT-dyad analysis to all five positive controls** \

Prepare the positive control files in fasta format. \
```{r engine='bash', eval=F, echo=TRUE}
module load bedtools
genome=/home/FCAM/ssun/Genome/hg38.fa
dir=/home/FCAM/ssun/GATA3_ChIP_PRO_July2023/ChIP_final/GAT_3mer_analaysis/MAST_positive_control/
fastaFromBed -fi $genome -bed ${dir}GATA3_peak_161win_with_motif_1.bed -fo GATA3_peak_161win_with_motif_1.fasta
fastaFromBed -fi $genome -bed ${dir}GATA3_peak_161win_with_motif_2.bed -fo GATA3_peak_161win_with_motif_2.fasta
fastaFromBed -fi $genome -bed ${dir}GATA3_peak_161win_with_motif_4.bed -fo GATA3_peak_161win_with_motif_4.fasta
fastaFromBed -fi $genome -bed ${dir}GATA3_peak_161win_with_motif_5.bed -fo GATA3_peak_161win_with_motif_5.fasta
fastaFromBed -fi $genome -bed ${dir}GATA3_peak_161win_with_motif_6.bed -fo GATA3_peak_161win_with_motif_6.fasta
```
...
# -2str	count on oth strands; The occurrences of each oligonucleotide are summed on both strands. This allows to detect elements which act in an orientation-insensitive way (as is generally the case for yeast upstream elements).
...
wc -l GATA3_peak_161win_with_motif_1_RSAT_dyad.txt #43681 GATA3_peak_161win_with_motif_1_RSAT_dyad.txt
head GATA3_peak_161win_with_motif_1_RSAT_dyad.txt 
#sequence  	identifier             	expected_freq	occ	exp_occ	ovl_occ	all_occ	ratio
...
#gatn{1}tta	gatn{1}tta|taan{1}atc	0.0002442959088	   6255	 300.53	50	6305	  20.81
#agan{5}tca	agan{5}tca|tgan{5}tct	0.0006445764392	   5986	 756.26	43	6029	   7.92
#attn{2}atc	attn{2}atc|gatn{2}aat	0.0002487059113	   5956	 301.58	690	6646	  19.75
#atgn{2}atc	atgn{2}atc|gatn{2}cat	0.0003617165575	   5268	 438.62	51	5319	  12.01
```

Other positive controls: \
```{r engine='bash', eval=F, echo=TRUE}
rsat dyad-analysis -o GATA3_peak_161win_with_motif_2_RSAT_dyad.txt -i GATA3_peak_161win_with_motif_2.fasta -format FastA -l 3 -sp 0-20 -expfreq ENCODE.MCF7.DHS.background4.txt -return exp_occ,occ,ratio -sort -seqtype dna 

rsat dyad-analysis -o GATA3_peak_161win_with_motif_4_RSAT_dyad.txt -i GATA3_peak_161win_with_motif_4.fasta -format FastA -l 3 -sp 0-20 -expfreq ENCODE.MCF7.DHS.background4.txt -return exp_occ,occ,ratio -sort -seqtype dna 

rsat dyad-analysis -o GATA3_peak_161win_with_motif_5_RSAT_dyad.txt -i GATA3_peak_161win_with_motif_5.fasta -format FastA -l 3 -sp 0-20 -expfreq ENCODE.MCF7.DHS.background4.txt -return exp_occ,occ,ratio -sort -seqtype dna 

rsat dyad-analysis -o GATA3_peak_161win_with_motif_6_RSAT_dyad.txt -i GATA3_peak_161win_with_motif_6.fasta -format FastA -l 3 -sp 0-20 -expfreq ENCODE.MCF7.DHS.background4.txt -return exp_occ,occ,ratio -sort -seqtype dna 
```

**Rank the output patterns** \

The last column ratio is the observed/expected occurrence ratio. This ratio can be used as a rough indicator of over-representation, but it might overestimate the patterns with a very low number of expected occurrences. \

I will rank the patterns by ratio, and first find the patterns associated with GAT/ATC (that appear in both dyad). \




**glk-SVM** \
Suitable for finding co-binding (individual binding elements with several spacing to another individual binding elements) (happens when protein pairs are found; CAP-SELEX) \
https://github.com/kevingroup/glk-SVM \
https://academic.oup.com/bib/article/21/5/1787/5612164 \
