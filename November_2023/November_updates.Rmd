...
```
These collapsed region won't affect our downstream analysis. \
...
Most entry in minus_ATC.bed and plus_GAT.bed are overlapped, however, there are some different entries in both file. See example below: \
```{r engine='bash', eval=TRUE, echo=TRUE}
...
**Why seqoutbias generate different 3mer coordinates for reverse complimented 3mer??**
...
**Input 1** A peak summit file. This file was originally generated with `Macs3` during peak calling step, then processed by removing peaks on contigs and within blacklisted regions, finally we use `slopBed -b 50` to make 50bp window at up/downstream of the summit. \
```{r engine='bash', eval=T, echo=TRUE}
...
**In the above analysis, we choose a arbitrary 100bp window to query for peaks with motif, this is risky because we might lose information if motif exceeds this window. A better way to decide the summit window is: first use `closestBed` to find peak summit (1bp) to the closest motif; then plot the distribution of closest motif distance to summit; we also want to plot the peak intensity. Based on these analysis, we can choose an appropriate window. (do this later)**
...
```{r  fig.align = "center", out.width="50%", out.height="50%", echo=F, fig.align = "center", fig.cap="closest upstream GAT to peak summit"}
library(knitr)
knitr::include_graphics("./upstream_closest.png") 
```

```{r  fig.align = "center", out.width="50%", out.height="50%", echo=F, fig.align = "center", fig.cap="closest downstream GAT to peak summit"}
library(knitr)
