...
In this dump.txt file, the first column is chromosome region (of each base position), the second and third columns are index and reversed index. We see many 0s in 2nd and 3rd columns in the above headed file, because these are Ns in the beginning of chromosome genome file. These Ns comes from repetitive DNA sequences such as telomeres and centromeres that is hard for sequencer to recognonize the exact base. \
...
<span class="highlighted"> A coherence check here could be: </span> \
<span class="highlighted"> word count the original .bed file (3mer coordinates), and confirm that every coordinates entry in that file has 3 bp distance between chromosome start and end: </span> \
```{r engine='bash', class.source = "revision", eval=TRUE, echo=TRUE}
...
<span class="highlighted">Then we can run `awk` on .bedGraph file to output all coordinate entry that spans larger than 3bp between chr start and end. </span> \
<span class="highlighted">We can calculate the sum of these spanned distance, then calculate how many 3mer regions are collapsed together by dividing this sum of distance by 3. </span> \
```{r engine='bash', class.source = "revision", eval=TRUE, echo=TRUE}
...
```
<span class="highlighted">These collapsed region won't affect our downstream analysis.</span> \

<span class="highlighted">The final step is to convert .bedGraph to .bigWig with `UCSC bedGraphToBigWig` tool.</span> \
```{r engine='bash', class.source = "revision", eval=F, echo=TRUE}
...
<span class="highlighted">Most entry in minus_ATC.bed and plus_GAT.bed are overlapped, however, there are some different entries in both file. See example below:</span> \
```{r engine='bash', class.source = "revision", eval=TRUE, echo=TRUE}
...
We also use `MAST` to find this unique motif's occurrence in ChIP peak. The motif coordinates information is stored in "mast_GATA3_PSWM_in_peaks_round2.bed".  \
...
<span class="highlighted">**Input 1** A peak summit file. This file was originally generated with `Macs3` during peak calling step, then processed by removing peaks on contigs and within blacklisted regions, finally we use `slopBed -b 50` to make 50bp window at up/downstream of the summit.</span> \
```{r engine='bash', class.source = "revision", eval=T, echo=TRUE}
...
<span class="highlighted">**Input 2**  motif occurence in peak region.</span> \
```{r engine='bash', class.source = "revision", eval=T, echo=TRUE}
...
<span class="highlighted">This bed file stores peak information of chr, start, end, peak-index, intensity. The peak coordinates have a 100bp window, we will center them to 1bp summit later.</span> \
```{r engine='bash', class.source = "revision", eval=TRUE, echo=TRUE}
head with_motif_2_100win.bed 
```

<span class="highlighted">**In the above analysis, we choose a arbitrary 100bp window to query for peaks with motif, this is risky because we might lose information if motif exceeds this window. A better way to decide the summit window is: first use `closestBed` to find peak summit (1bp) to the closest motif; then plot the distribution of closest motif distance to summit; we also want to plot the peak intensity. Based on these analysis, we can choose an appropriate window. (do this later)**</span> \
...
<span class="highlighted">Parameter `-d` will report the distance from the closest GAT to the peak summit.</span> \
<span class="highlighted">Parameter `-t last` or `-t first` will only report either the last or the first entry in bed2 file if tied distance occurred.</span> \
<span class="highlighted">Here we choose to use `-t first` to report the first coordinates when tie occurred.</span> \
...
<span class="highlighted">**coherence check 1**: the number of the closest minus GAT is comparable to the number of the closest plus GAT.</span> \
...
<span class="highlighted">**coherence check 2**: Since we have use `-t first` to take care of the tie, now we have equal number of unique peak with their closest plus or minus or both GAT.</span> \
```{r engine='R', class.source = "revision",eval=TRUE, echo=TRUE}
...
<span class="highlighted">Previously, we have made a UCSC genome browser trackhub (http://guertinlab.cam.uchc.edu/test_hub/hub.txt) that has the 3mer coordinates region on ref genome.</span> \
<span class="highlighted">We will also upload custome tracks of 1) peak summit file; and 2) closestBed defined closest GAT coordinates (plus, minus, all).</span> \
```{r engine='R', class.source = "revision", eval=TRUE, echo=TRUE}
#1)peak summit
...
<span class="highlighted">Add trackline:</span> \
```{r engine='bash', class.source = "revision", eval=TRUE, echo=TRUE}
...
<span class="highlighted">get peak region where the closest plus GAT and closest minus GAT has the same distance to peak summit defined by `bedtools closestBed`.</span> \
```{r engine='R', class.source = "revision", eval=TRUE, echo=TRUE}
head(plus.distance[plus.distance$V11 == minus.distance$V11, c('V1', 'V2', 'V3', "V11")])
```
<span class="highlighted">Use peak summit at chr10:71850201-71850202 as an example, plus GAT and minus GAT has same distance =3 reported by `bedtools closestBed`. </span> \
```{r engine='R', class.source = "revision", eval=TRUE, echo=TRUE}
...
```{r engine='R', eval=TRUE, echo=TRUE}
Anchor.matrix = bed6.step.bpQuery.bigWig(bwMinus_GAT, bwPlus_GAT, bedAnchorwindow,
                 step = 1, as.matrix=TRUE, follow.strand=TRUE)
...
Anchor.matrix = bed6.step.bpQuery.bigWig(bwMinus_ATC, bwPlus_ATC, bedAnchorwindow,
                 step = 1, as.matrix=TRUE, follow.strand=TRUE)
