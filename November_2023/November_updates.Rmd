...
Install `closestBed` \
```{r engine='bash', eval=TRUE, echo=F}
...
`intersectBed -wa` reports original entry in A for each overlap. \
```{r engine='bash', eval=F, echo=TRUE}
module load bedtools
...
```{r engine='R', eval=TRUE, echo=TRUE}
# define function 
bedTools.closest <- function(functionstring="closestBed",bed1,bed2,opt.string="") {
...
```{r engine='R', eval=TRUE, echo=TRUE}
nrow(all.distance)
nrow(all.distance[all.distance$V6=="+",])
nrow(all.distance[all.distance$V6=="-",])
```
...
#bed6-plus
bed6_plus=bed6[bed6$V6=="+",]
#bed6-minus
bed6_minus=bed6[bed6$V6=="-",]
...
head(tss.matrix)
```
`follow.strand` not invoked. Since `follow.strand` only affect - strand output, the +strand outputs are not affected by this parameter. \
```{r engine='R', eval=TRUE, echo=TRUE}
tss.matrix = bed6.step.bpQuery.bigWig(bwPlus, bwMinus, bedTSSwindow,
